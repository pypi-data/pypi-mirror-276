...
class RIPCCSD1(RSIPCC1):
    ...
    @timer.with_section("Subspace H IPCCSD1")
    ...
    def get_2_hole_r_i_terms(self, b_2aa, b_2bb, s_1):
        """Determine all contributions containing two hole operators:
            * coupling terms to R_i
    ...
    @timer.with_section("SetH IPCCSD1")
    ...
    def set_hamiltonian_2_hole(self, fock, mo2):
        ...
        **Arguments:**
        ...
        # x2ijbm
        #
        self.set_hamiltonian_x2ijbm(fock, goooo, gooov, mo2)
        #
        # xiJkL
        #
    ...
    def set_hamiltonian_x1im(self, fock, mo2):
        ...
        **Arguments:**
        ...
        # a) connected terms
        # -tid fmd
        t_1.contract("ab,cb->ac", fock, x1im, factor=-1.0, end2=no, begin3=no)
        # - (2<ml|id> - <lm|id>) tld
    ...
    def set_hamiltonian_xkc(self, fock, mo2):
    ...
    def set_hamiltonian_x4bd(self, fock, mo2):
        ...
        **Arguments:**
        ...
        # -fkc tkb
        t_1.contract("ab,ac->bc", fock, x4bd, factor=-1.0, end2=no, begin3=no)
        # 2<bk|dc> - <bk|cd>) tkc
        ...
        if not self.disconnected_t1:
            # skip remaining code
            return
        # -(2<mk|dc> - <mk|cd>) tmb tkc -> <mk|dc>/<mk|cd> tmb tkc
        # (dm) <mk|dc>/<mk|cd> tkc
    ...
    def set_hamiltonian_xiJBm(self, fock, goooo, gooov, mo2):
    ...
    def set_hamiltonian_x2ijbm(self, fock, goooo, gooov, mo2):
        ...
        **Arguments:**
        ...
        # 1-2) 0.5 <ij||mk> tkb
        goooo.contract("abcd,de->abec", t_1, x2ijbm, factor=0.5)
        goooo.contract("abcd,ce->abed", t_1, x2ijbm, factor=-0.5)
        # 1-3) -<ic||mb> tjc -> <ib|mc>/<im|bc> tjc
        mo2.contract("abcd,ed->aebc", t_1, x2ijbm, factor=-1.0, **ovov)
        mo2.contract("abcd,ed->aecb", t_1, x2ijbm, factor=1.0, **oovv)
        ...
        #
        # a) get T1T2 part of x2ijbm intermediate
        #
        self.get_t1_t2_x2ijbm(x2ijbm, mo2)
        #
        # b) disconnected terms: T1.T1
        #
        if not self.disconnected_t1:
            # do not calculate T1.T1 and T1.T1.T1 contributions
            return
        # 1-8) -0.5 <mb||cd> tic tjd
        # (ibmd)
        tmp = mo2.contract("abcd,ec->ebad", t_1, factor=-0.5, **ovvv)
        mo2.contract("abcd,ed->ebac", t_1, tmp, factor=0.5, **ovvv)
        # (ibmd) tjd
        tmp.contract("abcd,ed->aebc", t_1, x2ijbm, factor=1.0)
        del tmp
        # 1-7) <km||jc> tic tkb
        # (ijmk)
        tmp = gooov.contract("abcd,ed->ecba", t_1, factor=1.0)
        gooov.contract("abcd,ed->ecab", t_1, tmp, factor=-1.0)
        # (ijmk) tkb
        tmp.contract("abcd,de->abec", t_1, x2ijbm)
        del tmp
        # 1-12) 0.5 <mk||cd> tic tjd tkb
        # (imkd) = <mk||cd> tic
        tmp = mo2.contract("abcd,ec->eabd", t_1, factor=0.5, **oovv)
        mo2.contract("abcd,ed->eabc", t_1, tmp, factor=-0.5, **oovv)
        # (ijmk) = (imkd) tjd
        tmp = tmp.contract("abcd,ed->aebc", t_1)
        # (ijmk) tkb
        tmp.contract("abcd,de->abec", t_1, x2ijbm, factor=1)
        del tmp

    def set_hamiltonian_ximKC(self, gooov, mo2):
        ...
        **Arguments:**
        ...
        # -<km|cd> tid
        mo2.contract("abcd,ed->ebac", t_1, ximKC, factor=-1.0, **oovv)
    ...
    def set_hamiltonian_ximkc(self, gooov, mo2):
        ...
        **Arguments:**
        ...
        # -0.5 <km||cd> tid
        mo2.contract("abcd,ed->ebac", t_1, ximkc, factor=-0.5, **oovv)
        mo2.contract("abcd,ed->eabc", t_1, ximkc, factor=0.5, **oovv)

    def set_hamiltonian_xiJkL(self, goooo, gooov, mo2):
    ...
    def set_hamiltonian_xijkl(self, goooo, gooov, mo2):
        ...
        **Arguments:**
        ...
        # 0.5 <lk||jc> tic
        gooov.contract("abcd,ed->ecba", t_1, xijkl, factor=0.5)
        gooov.contract("abcd,ed->ecab", t_1, xijkl, factor=-0.5)
        ...
        # 0.25 <kl||cd> tic tjd
        # (ikld) = <kl||cd> tic
        tmp = mo2.contract("abcd,ec->eabd", t_1, factor=0.25, **oovv)
        mo2.contract("abcd,ec->ebad", t_1, tmp, factor=-0.25, **oovv)
        # (ikld) tjd
        tmp.contract("abcd,ed->aebc", t_1, xijkl, factor=1)
        del tmp

    def set_hamiltonian_xiBkC(self, gooov, mo2):
    ...
    def set_hamiltonian_xjbKC(self, gooov, mo2):
        """Derive effective Hamiltonian term for xiBkC intermediate
        ...
        **Arguments:**
        ...
        # 5-7) <kb|cd> tjd
        mo2.contract("abcd,ed->ebac", t_1, xjbKC, **ovvv)
        #
        # a) disconnected terms: T1.T1
        #
        if not self.disconnected_t1:
    ...
    def get_t1_t2_xiJBm(self, xiJBm, mo2):
        ...
        **Arguments:**
        ...
        t_1 = self.checkpoint["t_1"]
        t_2 = self.checkpoint["t_2"]
        # 1-9) - <km|cd> tid (tjbkc - tjckb)
        # (ikmc) = -<km|cd> tid
        tmp = mo2.contract("abcd,ed->eabc", t_1, factor=-1.0, **oovv)
        # (ikmc) (tjbkc - tjckb)
        tmp.contract("abcd,efbd->aefc", t_2, xiJBm)
        tmp.contract("abcd,edbf->aefc", t_2, xiJBm, factor=-1.0)
        # 1-9) - <km||cd> tid tjbkc
        # (ikmc) + <km|dc> tid
        ...
        del tmp
        # 1-10) (-2<km|cd> tkc + <km|dc> tkc) tidjb
    ...
    def get_t1_t2_x2ijbm(self, x2ijbm, mo2):
        """Get T1T2 part of effective Hamiltonian term for x2ijbm intermediate
        ...
        **Arguments:**
        ...
        t_1 = self.checkpoint["t_1"]
        t_2 = self.checkpoint["t_2"]
        # 1-9) (-<km|cd> tid + <km|dc> tid) (tjbkc - tkbjc)
        # (ikmc) = - <km|cd> tid + <km|dc> tid
        tmp = mo2.contract("abcd,ed->eabc", t_1, factor=-1.0, **oovv)
        mo2.contract("abcd,ec->eabd", t_1, tmp, factor=1.0, **oovv)
        # (ikmc) (tjbkc - tkbjc)
        tmp.contract("abcd,efbd->aefc", t_2, x2ijbm, factor=1.0)
        tmp.contract("abcd,bfed->aefc", t_2, x2ijbm, factor=-1.0)
        # 1-9) -<km|cd> tid tjbkc
        # - <km|cd> tid
        tmp = mo2.contract("abcd,ed->eabc", t_1, factor=-1.0, **oovv)
        # (ikmc) tjbkc
        tmp.contract("abcd,efbd->aefc", t_2, x2ijbm, factor=1.0)
        del tmp
        # 1-10) - 0.5 (2 <km|cd> tkc + <km|dc> tkc) (tidjb - tibjd)
        # (md) = 2 <km|cd> tkc - <km|dc> tkc
        tmp = mo2.contract("abcd,ac->bd", t_1, factor=2.0, **oovv)
        mo2.contract("abcd,ad->bc", t_1, tmp, factor=-1.0, **oovv)
        # - 0.5 (md) (tidjb - tibjd)
        t_2.contract("abcd,eb->acde", tmp, x2ijbm, factor=-0.5)
        t_2.contract("abcd,ed->acbe", tmp, x2ijbm, factor=0.5)
        del tmp
        # 1-11) 0.25 <mk||cd> (ticjd-tjcid) tkb
        # (ijmk) = 0.25 <mk||cd> (ticjd-tidjc) = 0.5 <mk||cd> ticjd
        tmp = mo2.contract("abcd,ecfd->efab", t_2, factor=0.5, **oovv)
        mo2.contract("abcd,edfc->efab", t_2, tmp, factor=-0.5, **oovv)
        # (ijmk) tkb
        tmp.contract("abcd,de->abec", t_1, x2ijbm)
        del tmp


class RIPLCCSD1(RIPCCSD1):
    """
    Restricted Single Ionization Potential Equation of Motion Coupled Cluster
    class restricted to Single IP for a LCCSD reference function
    and 1 unpaired electron (S_z = 0.5)

    This class (re)defines only the function that are unique for the IP-LCCSD
...
class RIPfpLCCSD1(RIPCCSD1):
    """
    Restricted Single Ionization Potential Equation of Motion Coupled Cluster
    class restricted to Single IP for a fpLCCSD/pCCD-LCCSD reference function
    and 1 unpaired electron (S_z = 0.5)

    This class (re)defines only the function that are unique for the IP-fpLCCSD
    model with 1 unpaired electron:
    ...
    def get_t1_t2_xiJBm(self, xiJBm, mo2):
        ...
        **Arguments:**
        ...
        del tmp
        xiJBm.iadd_expand_three_to_four("abc->aabc", tmp_iibm)
    ...
    def get_t1_t2_x2ijbm(self, x2ijbm, mo2):
        ...
        **Arguments:**
        ...
        tmp = mo2.contract("abcd,ed->eacb", t_1, factor=-1.0, **oovv)
        # (ijbm) tjbjb
        tmp.contract("abcd,bc->abcd", t_p, x2ijbm, factor=1.0)
        del tmp
