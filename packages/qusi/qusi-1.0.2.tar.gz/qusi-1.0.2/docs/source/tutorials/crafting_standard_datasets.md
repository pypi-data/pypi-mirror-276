...
In the previous tutorials, you may have noticed that during the training we used a `LightCurveDataset`, but during inference a `FiniteStandardLightCurveDataset` was used. The `LightCurveDataset` designed to produce an infinite number of samples. This is done for a variety of reasons. First, the `LightCurveDataset` will balance the samples coming from each of its constituent `LightCurveCollection` components. TODO: continue writing why and for injected