...
for lang, model in spacy_models.items():
    try:
        ...
        print(f"The model '{model}' was not found. Loading...")
        download(model)
        nlp = spacy.load(model)

def preprocess_text_alternative(text, language):
    """
    Eng: Preprocessing of the text in the selected language.
    Fra: Prétraitement du texte dans la langue sélectionnée.
    Rus: Предварительная обработка текста на выбранном языке.
    Ger: Vorverarbeitung des Textes in der ausgewählten Sprache.
    
    Parameters:
    - text (str): The text to be processed.
    - language (str): The language of the text. Must be one of: 'english', 'german', 'russian', 'french'.
    
    Returns:
    - str: Preprocessed text.
    
    The processing includes:
    - Removing all unnecessary characters except letters and spaces.
    - Tokenization of the text.
    - Removing stop words.
    - - Lemmatization of the text.
    - Removing special characters.
    - Highlighting significant parts of speech (for example, nouns, verbs and adjectives).
    """
    if language not in spacy_models:
        raise ValueError("Unsupported language. Choose from 'english', 'german', 'russian', 'french'.")
    
    nlp = spacy.load(spacy_models[language])
    
    text = re.sub(r'[^a-zA-Zа-яА-ЯёЁäöüßÄÖÜâàçéèêëîïôûùÇÉÊÈËÎÏÔÛÙæœÆŒ\s]', '', text)
    ...
    lemmas = [re.sub(r'[^a-zA-Zа-яА-ЯёЁäöüßÄÖÜâàçéèêëîïôûùÇÉÊÈËÎÏÔÛÙæœÆŒ\s]', '', lemma) for lemma in lemmas]
    
    pos_tags = ['NOUN', 'VERB', 'ADJ']
    significant_tokens = [token.text for token in doc if token.pos_ in pos_tags]
    
    preprocessed_text = ' '.join(significant_tokens)
    
    return preprocessed_text

def preprocess_dataframe_alternative(df, text_column, language):
    """
    Eng: Preprocessing of a text column in a DataFrame in the selected language.
    Fra: Prétraitement d'une colonne de texte dans une trame de données dans la langue sélectionnée.
    Rus: Предварительная обработка текстового столбца во фрейме данных на выбранном языке.
    Ger: Vorverarbeitung einer Textspalte in einem Datenrahmen in der ausgewählten Sprache.
    
    Parameters:
    - - df (pandas.Data Frame): A data frame with a text column to process.
    - text_column (str): The name of the column with the text to be processed.
    - language (str): The language of the text. Must be one of: 'english', 'german', 'russian', 'french'.
    
    Returns:
    - - pandas.Data Frame: A data frame with preprocessed text in the specified column.
    ...
    - Highlighting significant parts of speech (for example, nouns, verbs and adjectives).
    """
    df[text_column] = df[text_column].apply(lambda text: preprocess_text(text, language))
    return df
