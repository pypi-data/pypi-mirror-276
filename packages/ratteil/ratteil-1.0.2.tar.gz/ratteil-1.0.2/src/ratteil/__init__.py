def ratteil():
  ...
  def PRE_LUNCHER(options, RATTEIL_RESOURCES):
    ...
    if options.reciter:
      ...
      while len(temp_reciters_list) != options.reciters_number: # to fix random randrange(1,1) issue
        temp_reciters_list.append(r.choice(options.reciter))
    else:
      temp_reciters_list = recitersCodesDB
  
    if not options.no_download:
      streamer.generate_random_reciters_list(options.reciters_number, temp_reciters_list,one_reciter)
  ...
  parser.add_argument("-t","--sites", metavar="", help="site to stream to", dest="sites", choices=["youtube"], nargs="*")
