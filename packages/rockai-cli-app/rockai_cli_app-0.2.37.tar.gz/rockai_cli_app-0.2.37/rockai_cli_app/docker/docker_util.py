...
def build_cpu_image(image_tag, config_map, tag, platform="linux/amd64", port=5000):
    # sed -i s@/archive.ubuntu.com/@/mirrors.aliyun.com/@g /etc/apt/sources.list
    # sed -i s@/security.ubuntu.com/@/mirrors.aliyun.com/@g /etc/apt/sources.list
    # apt-get clean
    # apt-get update
...
def build_final_image(config_map, port=5000):
    if config_map["build"]["gpu"] is True:
        ...
        # TODO
        build_docker_image("python:{}-alpine".format(py_version), config_map, config_map["image"], port=port)
