...
class MinecraftPlayer:
    ...
    def __post_init__(self):
        self.name = self._data.get("username", None)
        self.formattedUUID = self._data.get("uuid", None)
        self.uuid = self.formattedUUID.replace("-", "") if self.formattedUUID else None

@dataclass
class VoxylPlayer:
    _gen_data: dict = field(repr=False)
    _over_data: dict = field(repr=False)
    _game_data: dict = field(repr=False)
    _guild_data: dict = field(repr=False)
    _ach_data: dict = field(repr=False)
    uuid: str = None
    name: str = None
    ...
    def __post_init__(self):
        data = alias_convert({**self._gen_data, **self._over_data, **self._guild_data}, "PLAYER")
        for i in data:
            setattr(self, i, data[i])

        total = calculate_total_stats(self._game_data)
        ...
        for game, model in games.items():
            data = alias_convert(self._game_data, game, model)
            setattr(self, game, model(**data))

        self.achievements = self._ach_data
...
class VoxylPlayerInfo:
    ...
    def __post_init__(self):
        data = alias_convert(self._data, "PLAYER")
        for i in data:
            setattr(self, i, data[i])
...
class VoxylPlayerGames:
    ...
    def __post_init__(self):
        for game, model in games.items():
            data = alias_convert(self._data, game, model)
            setattr(self, game, model(**data))
...
class StatsLeaderboardPlayer:
    ...
    def __post_init__(self):
        data = alias_convert(self._data, "LEADERBOARD")
        for i in data:
            setattr(self, i, data[i])
...
class GuildMember:
    ...
    def __post_init__(self):
        data = alias_convert(self._data, "GUILD_MEMBER")
        for i in data:
            setattr(self, i, data[i])
