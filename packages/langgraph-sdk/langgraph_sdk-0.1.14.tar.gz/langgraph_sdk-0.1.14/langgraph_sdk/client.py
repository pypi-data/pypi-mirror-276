...
class ThreadsClient:
    ...
    async def create(
        self, *, metadata: Metadata = None, thread_id: Optional[str] = None
    ) -> Thread:
        """Create a new thread."""
    ...
    async def get_history(
        self, thread_id: str, limit: int = 10, before: Optional[str] = None
    ) -> list[ThreadState]:
...
class RunsClient:
    ...
    async def cancel(self, thread_id: str, run_id: str) -> None:
        """Get a run."""
        return await self.http.post(
            f"/threads/{thread_id}/runs/{run_id}/cancel", json=None
        )
