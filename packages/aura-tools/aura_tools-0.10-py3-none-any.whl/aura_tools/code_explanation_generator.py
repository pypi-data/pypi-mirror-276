...
class CodeExplanationGenerator:
    def __init__(self, client, result_file='outputs/result.json', error_file='outputs/error.json', description="", prompt=""):
        self.client = client
    ...
    def _llm_explanation_function(self, code, prefix="", suffix="", prompt=""):
        prompt = prompt if prompt else "请分析输出这段代码的含义,并为它添加详细的注释说明:"
        """使用大模型解释函数代码"""
        messages = [
            {
                "role": "user",
        ...
        try:
            ...
            for func_index, (func_name, func_code) in enumerate(functions):
                ...
                if self._check_is_exist(func_code):
                    ...
                    continue
                prefix = f"这段函数来自于{file}"
                if self.description:
                    prefix = f"{prefix}; \n项目的基本信息描述是:{self.description}\n"
                explanation = self._llm_explanation_function(func_code, prefix=prefix)
                result = {
                    "code_file": file,
