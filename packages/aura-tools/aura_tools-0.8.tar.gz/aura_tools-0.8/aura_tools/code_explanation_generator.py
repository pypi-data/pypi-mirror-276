...
class CodeExplanationGenerator:
    ...
    def _llm_explanation_function(self, code, prefix="", suffix=""):
        """使用大模型解释函数代码"""
        messages = [
            {
                "role": "user",
                "content": f"{prefix}\n请分析输出这段代码的含义,并为它添加详细的注释说明:{code}\n{suffix}"
            }
        ]
        try:
            print(messages)
        ...
        try:
            ...
            for func_index, (func_name, func_code) in enumerate(functions):
                ...
                if self._check_is_exist(func_code):
                    print(f"已经翻译过这段代码 : file={file},func_name={func_name},func_code={func_code}")
                    continue
