...
class QueryData(object):
    ...
    def read_object_file(self, file):
        ...
        if self.object_storage_type == ObjectStorageType.OSS:
            ...
            return self.read_from_stream(stream)
        ...
        with pa.ipc.RecordBatchStreamReader(stream) as reader:
            try:
                while True:
                    ...
                    result.extend(process_each_batch(batch))
            except StopIteration:
                logger.info("reader got all record batches")
            return result
...
def get_real_result(source_data):
    result = []
    batches = [b for b in source_data.to_batches()]
    for batch in batches:
        result.extend(process_each_batch(batch))
    return result


def process_each_batch(batch):
    result = []
    ...
    def get_arrow_result(self, arrow_buffer):
        try:
            ...
            with pa.ipc.RecordBatchStreamReader(io.BytesIO(buffer)) as reader:
                ...
                ##########################################################
                return get_real_result(new_table)
                ##########################################################
