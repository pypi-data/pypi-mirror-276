...
class wxHandler:
   ...
   # 36.取消hook日志**
    ...
    # 45.确认收款**
    def confirmPayment(self, wxid, transcationId, transferId):
        url = f"{self.base_url}/api/?type=45"
        print("modify confirmPayment ")
        raise RuntimeError("modify confirmPayment then deleted me")
        payload = json.dumps({
            "wxid": wxid, # 转账人微信id，从hook的消息中获取
            "transcationId": transcationId, # 从hook的消息中获取对应的字段内容。
            "transferId": transferId # 从hook的消息中获取对应的字段内容。
        })
        headers = {
            'Content-Type': 'application/json'
        }
