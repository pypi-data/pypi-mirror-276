...
def seeded_sampler(samples: int, attributes: int, seed=0) -> np.ndarray:
    ...
    :param seed: [in args/kwargs for dist_sampler] seed to be used
...
def lhs(samples: int, attributes: int, *args, **kwargs) -> np.ndarray:
    ...
    :param args: [in args/kwargs for dist_sampler] Arguments passed to pyDOE2.lhs
    :param kwargs: [in args/kwargs for dist_sampler] Arguments passed to pyDOE2.lhs
    :returns: an array of floats between 0 and 1 to be transformed in dist_sampler
    """
...
def morris_sample(
    ...
    :param samples: number of samples that are used as the origins - NOT total number of samples
    :param attributes: number of parameters that are to be varied
    :param problem_dup: [in args/kwargs for dist_sampler] the problem that is used
    :param steps: [in args/kwargs for dist_sampler] how long the random walk should be for each point (the number of times a variable is changed)
    :param random_order: [in args/kwargs for dist_sampler] boolean, -> if true: randomizes the order of the variables to be cycled through, and rerandomizes every time all variables are cycled through
                                                                    -> if false: uses the original order
    :chooser: [in args/kwargs for dist_sampler] function, how to choose a random number between 0 and 1. random.random default, so a uniform distribution, must take no arguments
    ...
    if (
        steps is None
    ):  # by default the number of steps (new points generated) will be equal to the number of parameters
        steps = len(problem_dup.parameters)

    init_samples_np = pyDOE2.lhs(
        attributes, samples
    )  # creates the samples that are to be used as origins
    new_samples_np = np.empty([0, attributes], dtype=float)

    params = list(
        enumerate(copy.deepcopy(problem_dup.parameters))
    )  # creates a list of (parameter's location in row, parameter)

    for row in init_samples_np:
        row = np.expand_dims(row, 0)  # expand dims to allow appending later
        temp_samples_np = copy.deepcopy(row)

        for x in range(steps):
            if (
                random_order and x % len(params) == 0
            ):  # randomize the order if random_order
                randlib.shuffle(params)

            loc, parameter = params[x % len(params)]
            if isinstance(parameter.value_descriptors[0], CategoryParameter):
                number = chooser()
                while parameter.value_descriptors[0].sample(
                    number
                ) == parameter.value_descriptors[0].sample(row[0, loc]):
                    number = (
                        chooser()
                    )  # this skips the value of the categorical parameter already present in the origin point, unless that categorical parameter only has one value
                    if len(parameter.value_descriptors[0].options) == 1:
                        break
                row = point = copy.deepcopy(
                    row
                )  # Unlike neighborhood_sample, the point varied next is the point *after* this variable is changed
                point[0, loc] = number
                temp_samples_np = np.append(
                    temp_samples_np, point, axis=0
                )  # Appends the new point to the temporary lis

            elif isinstance(parameter.value_descriptors[0], RangeParameter):
                number = chooser()
                row = point = copy.deepcopy(
                    row
                )  # Unlike neighborhood_sample, the point varied next is the point *after* this variable is changed
        ...
        new_samples_np = np.append(
            new_samples_np, temp_samples_np, axis=0
        )  # Appends the neighborhood for each origin to the output
    return new_samples_np


def neighborhood_sample(
    samples: int, attributes: int, problem_dup: Problem, subdivisions: int = 2
):
    ...
    :param attributes: number of parameters that are to be varied
    :param problem_dup: [in args/kwargs for dist_sampler] the problem that is used
    :param subdivisions: [in args/kwargs for dist_sampler] for range parameters, this is the number of subdivisions (>= 2)
    ...
    for row in init_samples_np:
        temp_samples_np = np.expand_dims(
            copy.deepcopy(row), 0
        )  # expand dims to allow appending later

        for loc, parameter in params:
            if isinstance(parameter.value_descriptors[0], CategoryParameter):
                for x in range(len(parameter.value_descriptors[0].options)):
                    number = (x + 0.5) / len(
                        params
                    )  # This reverse engineers CategoryParameter.sample() to get one of each sample
                    if parameter.value_descriptors[0].sample(
                        number
                    ) == parameter.value_descriptors[0].sample(row[loc]):
                        continue  # this skips the value of the categorical parameter already present in the origin point
                    point = np.expand_dims(copy.deepcopy(row), 0)
                    point[0, loc] = number
                    temp_samples_np = np.append(
                        temp_samples_np, point, axis=0
                    )  # Appends the new point to the temporary list

            elif isinstance(parameter.value_descriptors[0], RangeParameter):
                for x in range(subdivisions):
                    number = x / (
                        subdivisions - 1
                    )  # This way the extremes will always be included (0 and 1)
                    point = np.expand_dims(copy.deepcopy(row), 0)
                    point[0, loc] = number
                    temp_samples_np = np.append(
                        temp_samples_np, point, axis=0
