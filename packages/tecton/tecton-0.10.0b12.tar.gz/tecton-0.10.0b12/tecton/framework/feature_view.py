...
class FeatureView(base_tecton_object.BaseTectonObject):
    ...
    # True if this Feature View was initialized with the schema or features parameter set.
    # TODO(TEC-19861): Clean this temporary workaround up.
    _used_explicit_schema: Optional[bool] = attrs.field(init=False)
    ...
    def _validate(self, indentation_level: int = 0) -> None:
        ...
        if not self._used_explicit_schema:
...
class MaterializedFeatureView(FeatureView):
    ...
    def _derive_schemas_with_specs(
        ...
        if self._use_feature_param():
            ...
            # This should not occur; we already checked for explicit schema earlier in validate()
            raise errors.SchemaRequired(self.name)
    ...
    def _create_unvalidated_feature_definition(
        ...
        schemas = self._derive_schemas_with_specs(entity_specs)
...
class BatchFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        self._used_explicit_schema = schema is not None or features is not None
