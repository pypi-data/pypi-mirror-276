...
async def get_json(wikidata_id):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(BASE_URL, params={
                ...
                dicts = json.loads(response_text)
...
async def Concept(wikidata_id):
    ...
    if isinstance(wikidata_id, str):
        _concept = await get_json(wikidata_id)
...
async def Instance(wikidata_id):
    concept = await Concept(wikidata_id)
    if concept:
        return concept()
    else:
        return None

# Example of how to run the Instance function with asyncio
if __name__ == '__main__':
    wikidata_id = 'Q144'  # Example wikidata ID for testing
    instance = asyncio.run(Instance(wikidata_id))
    print(instance)
