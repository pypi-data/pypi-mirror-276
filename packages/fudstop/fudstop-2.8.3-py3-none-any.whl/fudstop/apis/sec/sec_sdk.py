...
class SECSdk:
    ...
    async def get_filing_content(self):
        ...
        async with httpx.AsyncClient(headers=sec.headers) as client:
            for link in links:
                try:
                    response = await client.get(link)
...
async def main():
    sec = SECSdk()
    await sec.db.connect()  # Ensure this is an async call
    sec_links = await sec.get_filing_content()

    document_links = await sec.get_document_links(sec_links)
