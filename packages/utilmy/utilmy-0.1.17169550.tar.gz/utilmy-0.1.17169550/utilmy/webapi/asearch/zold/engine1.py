...
def create_qdrant_index_dense(dirin: str, client_url_path: str = None,
    ...
    dirin: str: path to the parquet file
    client_url_path: str: url path to the qdrant server
    coltext: str: column name of the text column
    model_id: str: name of the embedding model to use
    """
...
def _compute_sparse_vectors(texts: list, model_id: str = None):
    ...
    # for each tensor in the batch, get the indices of the non-zero elements
    ...
    # for each tensor in the batch, get the values of the non-zero elements
...
def create_qdrant_index_sparse(dirin: str, client_url_path: str = None,
    ...
    client_url_path: str: url path to the qdrant server
    coltext: str: column name of the text column
    model_id: str: name of the sparse embedding model to use
    """
    docs = pd_read_file(path_glob=dirin)
    if not client_url_path:
        client = QdrantClient("http://localhost:6333")
...
def search_tantivy_index(datapath: str = "", query: str = "", size: int = 10):
    ...
    datapath: str: path to the tantivy index
    query: str: query to search
    size: int: number of results to return
    :return: list of tuples (score, document) where document is a dictionary of the document fields
    """
    if not datapath:
        datapath = pathlib.Path(".") / ".tantivy_index"
...
def test_create_tantivy_index():
    ...
    results = search_tantivy_index(
        query="Generative Knowledge Graph Construction (KGC) refers to those methods that leverage the sequence-to-sequence framework for building knowledge graphs, which is flexible and can be adapted to widespread tasks.")
    print(results)
    assert len(results) > 0
...
if __name__ == "__main__":
    ...
    fire.Fire()
    # Way to test the qdrant dense function
    # python3 -u engine.py test_create_qdrant_index_dense
    # Way to test the qdrant sparse function
    # python3 -u engine.py test_create_qdrant_index_sparse
    # Way to test the tantivy function
...
# Define the filter for category "city" with value "London"
filter_ = models.Filter(
    must=[
        models.FieldCondition(
            key="city",
...
# Define the search query with the filter
...
# Perform the search on a specific collection
