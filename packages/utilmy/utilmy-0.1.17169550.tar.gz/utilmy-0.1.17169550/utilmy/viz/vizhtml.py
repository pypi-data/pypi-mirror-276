...
###################################################################################################
######### CytoscapeGraph ###############################################################################
class CytoscapeGraph(vi.htmlDoc):
    import networkx as nx
    import markov_clustering as mcl

    def __init__(self, df, cola="col_node1", colb="col_node2", coledge="col_edge", layout={}, **kwargs):
        self.data = self._create_data(df, cola, colb)
        self.graph = nx.from_pandas_edgelist(df, cola, colb, create_using=nx.Graph())

        super().__init__(**kwargs)

        self.head = self._default_head()
        self.html = self._default_body()
    
    def _default_head(self):
        # Set the default head
        return f"""
         <script src="https://cdnjs.cloudflare.com/ajax/libs/cytoscape/3.22.1/cytoscape.min.js"></script></head>
         <script src="https://unpkg.com/cytoscape-graph-clustering/cytoscape-graph-clustering.js"></script>                
         <style>
            #cy {{
                width: 90%;
                height: 80%;
    ...
    def _default_body(self):
        ...
        for edge in self.graph.edges():
            elements.append({'data': {'id': f'{edge[0]}{edge[1]}', 'source': str(edge[0]), 'target': str(edge[1])}})
    ...
    def set_layout(self, new_layout):
        # Change layout
        updated_layout = self.merge_dict(self.extract_layout(self.html), new_layout)
        self.html = self.html.replace(f"\"layout\": {json.dumps(self.extract_layout(self.html))}", f"\"layout\": {json.dumps(updated_layout)}")

    def set_style(self, new_style):
        # Change style
        updated_style = self.merge_dict(self.extract_style(self.html), new_style)
        self.html = self.html.replace(f"\"style\": {json.dumps(self.extract_style(self.html))}", f"\"style\": {json.dumps(updated_style)}")

    @staticmethod
    def merge_dict(old_style, new_style):
        return {**old_style, **new_style}

    @staticmethod
    def extract_layout(html_body):
        start = html_body.find("\"layout\": ") + len("\"layout\": ")
        end = html_body.find("}", html_body.find("\"layout\": ") + len("\"layout\": "))
        return json.loads(html_body[start:end+1])
    
    @staticmethod
    def extract_style(html_body):
        start = html_body.find("\"style\": ") + len("\"style\": ")
        end = html_body.find("}", html_body.find("\"style\": ") + len("\"style\": "))
        return json.loads(html_body[start:end+1])
    
    def create_plot(self):
        self.html += "\n\n" + self.tail
        self.head += "\n\n" + self.head
        # self.save('ztmp/test_network_cyto_clusters.html')
        self.open_browser()
    ...
    def mcl_fit(self):
        ...
        node_mapping = {i: node for i, node in enumerate(self.graph.nodes())}

        # Convert clusters to use node names instead of indices
        clusters = [[node_mapping[node] for node in cluster] for cluster in clusters]    
        return clusters

    def graph_cluster(self, algorithm='mcl'):
        # Cluster elements and plot graph
        if algorithm=='mcl':
            clusters = self.mcl_fit()
        else:
        ...
        for cluster_id, cluster in enumerate(clusters):
            for node in cluster:
                elements.append({'data': {'id': str(node), 'cluster': cluster_id}})
        ...
        cluster_colors = ['#FF4136', '#2ECC40', '#0074D9', '#FF851B', '#7FDBFF', '#B10DC9']

        self.html = f"""        
        <div id=\"cy\"></div>
        <script>
            var elements = {elements_json};
            var clusterColors = {str(cluster_colors).replace("'", '"')};
            var cy = cytoscape({{
            ...
            style: [{{
                    ...
                    style: {{
                        'background-color': function(ele) {{
                            return clusterColors[ele.data('cluster') % clusterColors.length];
                        }},
                        'label': 'data(id)',
                        'color': '#fff',
                        'text-valign': 'center',
