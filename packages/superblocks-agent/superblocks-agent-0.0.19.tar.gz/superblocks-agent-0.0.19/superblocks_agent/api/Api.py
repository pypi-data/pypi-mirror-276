...
class Api:
    ...
    def __init__(self, id_: str, *, config: Optional[ApiConfig] = None):
        ...
        Args:
            id_ (str): The API ID.
            config (Optional[superblocks_agent.api.Config]): The API configuration. Defaults to `None`.
        """
    ...
    def __to_proto_fetch(self, client_config: ClientConfig) -> ExecuteRequest.Fetch:
        ...
        if self.__config is not None and client_config is not None:
            profile_name = (
                self.__config.profile
                if self.__config.profile is not None
                else client_config.profile
            )
            ...
            branch_name = (
                ...
                else client_config.branch_name
            )
