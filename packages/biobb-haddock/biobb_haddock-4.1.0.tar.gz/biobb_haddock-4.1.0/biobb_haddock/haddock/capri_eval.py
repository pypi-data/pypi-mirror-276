...
class CapriEval(BiobbObject):
    ...
    def __init__(self, input_haddock_wf_data_zip: str, output_evaluation_zip_path: str,
        ...
        # Input/Output files
        self.io_dict = {
            "in": {"reference_pdb_path": reference_pdb_path,
    ...
    def launch(self) -> int:
        ...
        if self.stage_io_dict['in'].get('reference_pdb_path'):
            self.cfg['reference_fname'] = self.stage_io_dict['in'].get('reference_pdb_path')
        ...
        output_file_list = [str(path) for path in Path(haddock_output_list[0]).iterdir() if path.is_file() and str(path).endswith(('izone', 'aln', 'tsv'))]
        fu.zip_list(self.io_dict['out']['output_evaluation_zip_path'], output_file_list, self.out_log)
...
def capri_eval(input_haddock_wf_data_zip: str, output_evaluation_zip_path: str,
    ...
    return CapriEval(input_haddock_wf_data_zip=input_haddock_wf_data_zip,
                     output_evaluation_zip_path=output_evaluation_zip_path,
...
def main():
    parser = argparse.ArgumentParser(description="Wrapper of the haddock CapriEval module.",
    ...
    capri_eval(input_haddock_wf_data_zip=args.input_haddock_wf_data_zip,
