...
class TestDecodeBraceFile(unittest.TestCase):
    ...
    def test_decode_all_raw_data(self):
        ...
        test_files = ['привязки 1 нет объектов и привязок',
                      'привязки 3 привязки явно убраны',
                      'привязки 4 к форме по горизонтали слева',
                      'привязки 6 к форме по горизонтали справа']
        result = ''
        for file_name in test_files:
            if file_name == 'temp':
                continue
            # uuid, x = file_name.split('.')
            result += self.decode_raw_data(file_name)
        self.assertEqual('', result)

    def test_decode_file(self):
        uuid = 'привязки 3 привязки явно убраны'
