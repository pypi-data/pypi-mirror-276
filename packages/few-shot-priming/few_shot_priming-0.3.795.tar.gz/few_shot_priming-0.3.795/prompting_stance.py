...
def format_prompt(dataset, tokenizer, labels_text_map, token_limit, few_shot_size, logger, model_name=None, input=pd.DataFrame({})):
    ...
    if len(input):
        ...
        if "alpaca" in model_name:
            ...
            vicuna_head = "User: Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.\n"
            prompt = vicuna_head + prompt_head
        else:
            prompt = prompt_head
        ...
        if "alpaca" in model_name:
            ...
            input = f"On the topic '{topic}' the argument '{sentence}' has the stance \nAssistant:"
        else:
