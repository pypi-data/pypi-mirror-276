...
class Context:
    ...
    def _observe_context_metrics(self):
        ...
        if self.num_total_pinned_message_tokens > 0:
            TOKEN_METRICS.labels(metric_type="total_pinned_message_tokens").observe(
                self.num_total_pinned_message_tokens
            )
        ...
        if self.num_total_pinned_messages > 0:
            MESSAGE_METRICS.labels(metric_type="total_messages_with_pinned").observe(
                self.num_total_messages
            )
            MESSAGE_METRICS.labels(
                metric_type="truncated_messages_with_pinned"
            ).observe(self.num_messages_truncated)
            MESSAGE_METRICS.labels(metric_type="total_pinned_messages").observe(
                self.num_total_pinned_messages
            )
