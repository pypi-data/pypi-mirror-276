...
from kozmo_ai.api.utils import authenticate_client_and_token, has_at_least_editor_role
from kozmo_ai.data_preparation.models.errors import FileNotInProjectError
from kozmo_ai.data_preparation.models.file import ensure_file_is_in_project
...
from kozmo_ai.shared.array import find_index


class KozmoTermManager(terminado.NamedTermManager):
...
class KozmoUniqueTermManager(terminado.UniqueTermManager):
    ...
    def __initialize_terminal(self, term_name: str, cwd: str = None):
        ...
        if self.term_command == 'bash':
            self.terminal.ptyproc.write(
                "bind 'set enable-bracketed-paste off' # Kozmo terminal settings command\r")
