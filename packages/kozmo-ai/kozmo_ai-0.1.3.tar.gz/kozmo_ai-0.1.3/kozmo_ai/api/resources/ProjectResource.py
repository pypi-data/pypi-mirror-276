...
from kozmo_ai.shared.hash import combine_into, merge_dict
...
class ProjectResource(GenericResource):
    ...
    async def update(self, payload, **kwargs):
        ...
        data = {}
        should_log_project = self.model.get('help_improve_kozmo') or False
        ...
        if 'help_improve_kozmo' in payload:
            if payload['help_improve_kozmo']:
                should_log_project = True

                if not repo_config.project_uuid:
                    data['project_uuid'] = uuid.uuid4().hex

            data['help_improve_kozmo'] = payload['help_improve_kozmo']

        if 'deny_improve_kozmo' in payload:
            await UsageStatisticLogger().project_deny_improve_kozmo(
