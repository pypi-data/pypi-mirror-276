...
from kozmo_ai.data_preparation.models.block.sql.constants import (
...
KOZMO_SEMI_COLON = '__KOZMO_SEMI_COLON__'
...
def should_cache_data_from_upstream(
    ...
    if BlockType.DBT == block.type:
        # TODO : check to see if the upstream block has the same data source
        return True
...
def split_query_string(query_string: str) -> List[str]:
    ...
    for _, match in enumerate(matches):
        ...
        updated_string = re.sub(r';', KOZMO_SEMI_COLON, matched_string)
    ...
    for query in queries:
        ...
        query = query.strip()
        query = re.sub(KOZMO_SEMI_COLON, ';', query)
