...
from kozmo_ai.authentication.permissions.constants import EntityName, PermissionAccess
from kozmo_ai.data_preparation.models.pipeline import Pipeline
from kozmo_ai.orchestration.db.models.base import BaseModel
from kozmo_ai.orchestration.db.models.oauth import (
    Permission,
    Role,
    RolePermission,
    User,
...
from kozmo_ai.shared.strings import classify, singularize
from kozmo_ai.tests.base_test import AsyncDBTestCase
from kozmo_ai.tests.factory import create_pipeline_with_blocks
...
def get_policy(resource: str):
    ...
    return getattr(
        importlib.import_module(f'kozmo_ai.api.policies.{model_name}Policy'),
        f'{model_name}Policy',
    )
...
def get_resource(resource: str):
    model_name = entity_name(resource)
    return getattr(
        importlib.import_module(f'kozmo_ai.api.resources.{model_name}Resource'),
    ...
    async def build_test_list_endpoint(
        ...
        with patch_manager(patch_function_settings or []) as mocks:
            with patch_object_manager(patch_object_settings or []) as mock_objects:
                with patch(
                    'kozmo_ai.api.policies.BasePolicy.REQUIRE_USER_AUTHENTICATION',
                    authentication or permissions or 0,
                ):
                    with patch(
                        'kozmo_ai.api.policies.BasePolicy.REQUIRE_USER_PERMISSIONS',
                        permissions or 0,
                    ):
                        meta = None
                        if build_meta:
