...
from kozmo_ai.cli.utils import parse_runtime_variables
from kozmo_ai.data_preparation.repo_manager import ProjectType
from kozmo_ai.services.newrelic import initialize_new_relic
...
INIT_PROJECT_PATH_DEFAULT = typer.Argument(..., help='path of the Kozmo project to be created.')
...
START_PROJECT_PATH_DEFAULT = typer.Argument(
    os.getcwd(), help='path of the Kozmo project to be loaded.')
...
RUN_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Kozmo project that contains the pipeline.'
)
...
CLEAN_LOGS_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Kozmo project to clean old logs.'
)
...
CLEAN_VARIABLES_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Kozmo project to clean variables.'
)
...
CREATE_SPARK_CLUSTER_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Kozmo project that contains the EMR config.'
)
...
def init(
    ...
    """
    Initialize Kozmo project.
    """
    from kozmo_ai.data_preparation.repo_manager import init_repo
    ...
    print(f'Initialized Kozmo project at {repo_path}')
...
def start(
    ...
    """
    Start Kozmo server and UI.
    """
    from kozmo_ai.settings.repo import set_repo_path

    # Set repo_path before intializing the DB so that we can get correct db_connection_url
    project_path = os.path.abspath(project_path)
    set_repo_path(project_path)

    from kozmo_ai.server.server import start_server
...
def run(
    ...
    from kozmo_ai.orchestration.db.models.schedules import PipelineRun
    from kozmo_ai.orchestration.utils.git import log_git_sync, run_git_sync
    from kozmo_ai.server.logger import Logger
    from kozmo_ai.settings import SENTRY_DSN, SENTRY_TRACES_SAMPLE_RATE
    ...
    with newrelic.agent.BackgroundTask(application, name="kozmo-run", group='Task') \
        ...
        # "kozmo_secret_var" syntax is used in the project's metadata.yaml
...
def clean_cached_variables(
    ...
    from kozmo_ai.data_preparation.variable_manager import clean_variables
...
def clean_old_logs(
    ...
    from kozmo_ai.data_preparation.logging.logger_manager_factory import (
...
def create_spark_cluster(
    ...
    """
    Create EMR cluster for Kozmo project.
    """
    from kozmo_ai.services.aws.emr.launcher import create_cluster
