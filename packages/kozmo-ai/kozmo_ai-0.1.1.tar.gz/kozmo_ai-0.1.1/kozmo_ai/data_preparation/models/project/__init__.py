...
from kozmo_ai.cluster_manager.constants import KUBE_NAMESPACE, ClusterType
from kozmo_ai.data_preparation.models.project.constants import FeatureUUID
from kozmo_ai.data_preparation.repo_manager import get_cluster_type, get_repo_config
from kozmo_ai.server.constants import VERSION
...
class Project:
    ...
    def help_improve_kozmo(self) -> bool:
        return self.repo_config.help_improve_kozmo
    ...
    def platform_settings(self) -> Dict:
        if project_platform_activated():
            return platform_settings(kozmo_projects_only=True)
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    'https://pypi.org/pypi/kozmo-ai/json',
                    timeout=3,
                ) as response:
