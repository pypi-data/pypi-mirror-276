...
from kozmo_ai.orchestration.queue.config import QueueConfig, QueueType
...
class QueueFactory:
    ...
    def get_queue(self):
        ...
        if queue_config.queue_type == QueueType.CELERY:
            from kozmo_ai.orchestration.queue.celery_queue import CeleryQueue
            self.queue = CeleryQueue(queue_config)
        else:
            from kozmo_ai.orchestration.queue.process_queue import ProcessQueue
