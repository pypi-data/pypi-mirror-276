...
from kozmo_ai.orchestration.constants import DATABASE_CONNECTION_URL_ENV_VAR
from kozmo_ai.orchestration.db.cache import CachingQuery, SessionWithCaching
from kozmo_ai.orchestration.db.setup import get_postgres_connection_url
from kozmo_ai.orchestration.db.utils import get_user_info_from_db_connection_url
from kozmo_ai.settings import OTEL_EXPORTER_OTLP_ENDPOINT
...
if is_test():
    ...
    if pg_db_connection_url:
        ...
        if is_test():
            db_connection_url = f'sqlite:///{TEST_DB}'
        elif os.path.exists(os.path.join('kozmo_ai', 'orchestration', 'db')):
            # For local dev environment
            db_connection_url = 'sqlite:///kozmo_ai/orchestration/db/kozmoai.db'
        elif os.path.exists('kozmoai.db'):
            # For backward compatiblility
            db_connection_url = f'sqlite:///{get_variables_dir()}/kozmoai.db'
        else:
            # For new projects, create kozmoai.db in variables dir
            db_connection_url = f'sqlite:///{get_variables_dir()}/kozmoai.db'
        db_kwargs['connect_args']['check_same_thread'] = False
