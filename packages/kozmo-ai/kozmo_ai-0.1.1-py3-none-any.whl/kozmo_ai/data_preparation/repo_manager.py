...
from kozmo_ai.settings import INITIAL_METADATA, settings
from kozmo_ai.settings.repo import (
    DEFAULT_KOZMO_DATA_DIR,
    KOZMO_DATA_DIR_ENV_VAR,
    PROJECT_METADATA_FILENAME,
)
from kozmo_ai.settings.repo import get_data_dir as get_data_dir_new
from kozmo_ai.settings.repo import get_metadata_path
from kozmo_ai.settings.repo import get_repo_name as get_repo_name_new
from kozmo_ai.settings.repo import get_repo_path as get_repo_path_new
from kozmo_ai.settings.repo import get_variables_dir
from kozmo_ai.settings.repo import set_repo_path as set_repo_path_new
from kozmo_ai.settings.utils import base_repo_path
from kozmo_ai.shared.environments import is_debug
from kozmo_ai.shared.yaml import trim_strings
...
class RepoConfig:
    def __init__(
        ...
        try:
            ...
            self.help_improve_kozmo = repo_config.get('help_improve_kozmo')
    ...
    def pipelines(self, pipelines: Dict = None) -> None:
        from kozmo_ai.data_preparation.models.project.models import ProjectPipelines
    ...
    def to_dict(self, remote: bool = False) -> Dict:
        return dict(
            ...
            help_improve_kozmo=self.help_improve_kozmo,
...
def init_repo(
    ...
    if project_type == ProjectType.MAIN:
        ...
        os.makedirs(
            os.getenv(KOZMO_DATA_DIR_ENV_VAR) or DEFAULT_KOZMO_DATA_DIR,
            exist_ok=True,
        )
...
def get_project_type(repo_config: RepoConfig = None, repo_path: str = None) -> ProjectType:
    from kozmo_ai.settings.repo import KOZMO_PROJECT_TYPE_ENV_VAR

    try:
        project_type_from_env = os.getenv(KOZMO_PROJECT_TYPE_ENV_VAR)
...
def get_cluster_type(repo_path=None) -> Optional[ClusterType]:
    from kozmo_ai.settings.repo import KOZMO_CLUSTER_TYPE_ENV_VAR

    try:
        cluster_type_from_env = os.getenv(KOZMO_CLUSTER_TYPE_ENV_VAR)
...
def get_repo_path() -> str:
    warn(
        'repo_manager.get_repo_path is deprecated. Please use kozmo_ai.settings.repo.get_repo_path',
        DeprecationWarning,
        stacklevel=2,
    )
...
def set_repo_path(repo_path: str) -> None:
    warn(
        'repo_manager.set_repo_path is deprecated. Please use kozmo_ai.settings.repo.set_repo_path',
        DeprecationWarning,
        stacklevel=2,
    )
...
def get_repo_name() -> str:
    warn(
        'repo_manager.get_repo_name is deprecated. Please use kozmo_ai.settings.repo.get_repo_name',
        DeprecationWarning,
        stacklevel=2,
    )
...
def get_data_dir() -> str:
    warn(
        'repo_manager.get_data_dir is deprecated. Please use kozmo_ai.settings.repo.get_data_dir',
        DeprecationWarning,
        stacklevel=2,
    )
