...
from kozmo_ai.services.search.constants import SEARCH_TYPE_BLOCK_ACTION_OBJECTS
    ...
    async def create(self, payload: Dict, user, **kwargs):
        ...
        if query:
            if SEARCH_TYPE_BLOCK_ACTION_OBJECTS == search_type:
                from kozmo_ai.services.search.block_action_objects import search

                results = await search(query, ratio=ratio)

                # TODO : remove this when we unify pipeline types
