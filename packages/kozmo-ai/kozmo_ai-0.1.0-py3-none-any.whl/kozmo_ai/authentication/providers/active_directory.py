...
from kozmo_ai.authentication.oauth.constants import (
    ACTIVE_DIRECTORY_CLIENT_ID as ACTIVE_DIRECTORY_KOZMO_CLIENT_ID,
)
from kozmo_ai.authentication.oauth.constants import ProviderName
from kozmo_ai.authentication.providers.oauth import OauthProvider
from kozmo_ai.authentication.providers.sso import SsoProvider
from kozmo_ai.authentication.providers.utils import get_base_url
from kozmo_ai.server.logger import Logger
from kozmo_ai.settings import get_settings_value
from kozmo_ai.settings.keys import (
...
class ADProvider(SsoProvider, OauthProvider):
    ...
    def get_auth_url_response(self, redirect_uri: str = None, **kwargs) -> Dict:
        ...
        can either just set the ACTIVE_DIRECTORY_DIRECTORY_ID which will use the Kozmo application we
        set up in Azure. They can additionally set the ACTIVE_DIRECTORY_CLIENT_ID and
        ACTIVE_DIRECTORY_CLIENT_SECRET which will use their own Kozmo application in Azure.
        """
        ...
        if self.client_id:
            ...
            query = dict(
                client_id=ACTIVE_DIRECTORY_KOZMO_CLIENT_ID,
                redirect_uri=f'https://api.kozmo.ai/v1/oauth/{self.provider}',
                response_type='code',
                scope=self.scope,
                state=quote_plus(
                    json.dumps(
                ...
                try:
                    ...
                    for assignment in app_role_assignments.get('value'):
                        ...
                        if app_role and app_role in self.roles_mapping:
                            kozmo_roles.append(self.roles_mapping[app_role])
                except Exception:
                    logger.exception(
                        'Failed to map Active Directory roles to Kozmo roles.'
                    )

        return dict(
            email=userinfo_resp.get('userPrincipalName'),
            username=userinfo_resp.get('userPrincipalName'),
            user_roles=kozmo_roles,
        )
