...
from kozmo_ai.services.k8s.config import K8sExecutorConfig
...
class JobManager():
    ...
    def create_job_object(
        ...
        if k8s_config and k8s_config.container_config:
            k8s_config.container_config.setdefault('name', 'kozmo-data')
            pod_spec.containers = [
                merge_containers(
    ...
    def merge_pod_spec(self, pod_spec: V1PodSpec, command: str) -> V1PodSpec:
        ...
        kozmo_server_pod_spec = self.pod_config.spec
        pod_spec.volumes.extend(kozmo_server_pod_spec.volumes)
        if not pod_spec.tolerations:
            # If there're no tolerations override, use server's tolerations
            pod_spec.tolerations = kozmo_server_pod_spec.tolerations
        pod_spec.node_selector = kozmo_server_pod_spec.node_selector
        pod_spec.ikozmo_pull_secrets = pod_spec.ikozmo_pull_secrets if pod_spec.ikozmo_pull_secrets \
            else kozmo_server_pod_spec.ikozmo_pull_secrets
        return pod_spec
    ...
    def merge_container_spec(self, container_spec: V1Container, command: str) -> V1Container:
        ...
        """
        kozmo_server_container_spec = self.get_kozmo_server_container()
        container_spec.env = container_spec.env + \
            [item for item in kozmo_server_container_spec.env if item not in container_spec.env]
        container_spec.env_from = (container_spec.env_from or []) + \
            [item for item in (kozmo_server_container_spec.env_from or [])
             if item not in (container_spec.env_from or [])]
        container_spec.volume_mounts = container_spec.volume_mounts + \
            [item for item in kozmo_server_container_spec.volume_mounts
                if item not in container_spec.volume_mounts]
        container_spec.image = container_spec.image if container_spec.image else \
            kozmo_server_container_spec.image
        container_spec.command = command
        return container_spec

    def get_kozmo_server_container(self) -> V1Container:
