...
from kozmo_ai.orchestration.notification.config import (
    AlertOn,
    MessageTemplate,
    NotificationConfig,
)
from kozmo_ai.services.discord.discord import send_discord_message
from kozmo_ai.services.email.email import send_email
from kozmo_ai.services.google_chat.google_chat import send_google_chat_message
from kozmo_ai.services.opsgenie.opsgenie import send_opsgenie_alert
from kozmo_ai.services.slack.slack import send_slack_message
from kozmo_ai.services.teams.teams import send_teams_message
from kozmo_ai.services.telegram.telegram import send_telegram_message
from kozmo_ai.settings import DEFAULT_LOCALHOST_URL, KOZMO_PUBLIC_HOST
...
DEFAULT_MESSAGES = dict(
    ...
    failure=dict(
        title='Failed to run Kozmo pipeline {pipeline_uuid}',
        summary=(
    ...
    passed_sla=dict(
        title='SLA missed for Kozmo pipeline {{pipeline_uuid}}',
        summary=(
...
class NotificationSender:
    ...
    def __with_pipeline_run_url(self, text, pipeline, pipeline_run):
        if text is None:
            return text
        text = f'{text}\n'
        if os.getenv('ENV') != 'production' or KOZMO_PUBLIC_HOST != DEFAULT_LOCALHOST_URL:
            """
            Include the URL for the following cases
            1. Dev environment: Use the default localhost as host in URL
            2. Production environment: If KOZMO_PUBLIC_HOST is set, use it as host.
            """
    ...
    def __pipeline_run_url(pipeline, pipeline_run):
        return f'{KOZMO_PUBLIC_HOST}/pipelines/{pipeline.uuid}/runs/'\
               f'{pipeline_run.id}'
