...
def _create_function_instance(ctx, f):
    ...
    if f.type == 'Affine':
        ...
        function = F.Unique(
            ctx,
            flatten=f.unique_param.flatten,
            axis=f.unique_param.axis,
            sorted=f.unique_param.sorted,
            with_index=f.unique_param.with_index,
            with_inverse=f.unique_param.with_inverse,
            with_counts=f.unique_param.with_counts,
            )
    elif f.type == 'EyeLike':
        function = F.EyeLike(
            ctx,
            k=f.eye_like_param.k,
            )
    elif f.type == 'Mod2':
        function = F.Mod2(
            ctx,
            fmod=f.mod2_param.fmod,
            )
    elif f.type == 'BitShift':
        function = F.BitShift(
            ctx,
            direction=f.bit_shift_param.direction,
            )
    elif f.type == 'Einsum':
        function = F.Einsum(
            ctx,
            equation=f.einsum_param.equation,
            )
    else:
