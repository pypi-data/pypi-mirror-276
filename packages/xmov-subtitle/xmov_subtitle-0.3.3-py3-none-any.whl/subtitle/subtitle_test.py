...
class SubtitleParser:
    ...
    def parse_singe_text(self, char_durations, max_lens=None):
        logging.info(f'subtitle_version: VERSION')
        if max_lens is None:
            max_lens = {'cn': 24, 'en': 27}

        # debug 用
        raw_text = char_durations
        ...
        assert len(candidates) == len(languages)
        text_list = [self.clean_text(text, lang) for text, lang in zip(candidates, languages) if text]
        text_list = [text for text in text_list if text and not is_all_punctuation(text)]
        return text_list
...
if __name__ == '__main__':
    subtitle_parser = SubtitleParser(user_dict='user_dict.txt')
    in_path = 'test.txt'
    ...
    res = subtitle_parser.parse_singe_text(text, {'cn': 24, 'en': 27})

    for item in res:
        print(subtitle_parser.get_length(item), item)

    exit()
    res = collections.defaultdict(list)
    input_file = '字幕.xlsx'
    out_config = [({'cn': 24, 'en': 27}, '字幕横屏_out.xlsx'), ({'cn': 16, 'en': 14}, '字幕竖屏_out.xlsx')]
    total_len = 0
    start_time = time.time()
    for text_len, output_file in out_config:
        df: pd.DataFrame = pd.read_excel(input_file, header=0, na_filter=False, dtype=str)
        outs = []
        columns = ["全文", "字幕结果"]
        for _, row in tqdm(df.iterrows()):
            ...
            pattern = r'{([^:]*):[^}]*}'
            text = re.sub(pattern, r'\1', text)
            res = subtitle_parser.parse_singe_text(text, text_len)

            subtitle_str = '\n'.join(
                [str(subtitle_parser.get_length(item)) + '    ' + item for item in res])
            # subtitle_str = '\n'.join([item for item in res])
            outs.append([text, subtitle_str])
            for item in res:
                total_len += len(item)
        out_df = pd.DataFrame(outs, columns=columns)
        out_df.to_excel(output_file, sheet_name='字幕', index=False)
    total_time = time.time() - start_time
    print('每一百字耗时：{:.4f}s'.format(total_time / total_len * 100))
