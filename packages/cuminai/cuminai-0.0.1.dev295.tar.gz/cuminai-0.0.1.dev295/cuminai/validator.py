...
from cuminai.config import ChunkStrategy, Tag, KnowledgeSource
...
class Validator:
    ...
    def validate(self):
        ...
        if not hasattr(self._cuminfile, 'version') or \
            not isinstance(self._cuminfile.version, int) or \
            self._cuminfile.version <= 0:
        ...
        if not hasattr(self._cuminfile, 'tag') or \
            not isinstance(self._cuminfile.tag, dict):
        ...
        if any(not hasattr(sourcedetails, 'source') for sourcedetails in self._cuminfile.knowledge) or \
            any(not utils.isValidKnowledgeSource(sourcedetails, self._cuminfile.kind) for sourcedetails in self._cuminfile.knowledge):
        ...
        if any(hasattr(source, 'metadata') and any(not isinstance(tag, str) for tag in source.metadata['tags']) for source in self._cuminfile.knowledge):
            raise ValueError("::invalid knowledge:metadata:tags: metadata tags should be a list of string tags")
        
        if hasattr(self._cuminfile, 'chunkstrategy') and not isinstance(self._cuminfile.chunkstrategy, dict):
        ...
        if hasattr(self._cuminfile, 'chunkstrategy'):
            ...
            if not hasattr(self._cuminfile.chunkstrategy, 'size') or not isinstance(self._cuminfile.chunkstrategy.size, int) or \
            ...
            if not hasattr(self._cuminfile.chunkstrategy, 'overlap') or not isinstance(self._cuminfile.chunkstrategy.overlap, int) or \
                self._cuminfile.chunkstrategy.overlap < 0 or self._cuminfile.chunkstrategy.overlap > self._cuminfile.chunkstrategy.size/2:
