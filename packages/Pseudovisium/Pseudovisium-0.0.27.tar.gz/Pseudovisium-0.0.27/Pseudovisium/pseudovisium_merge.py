...
def merge_data(folders, pv_format=False):
    ...
    for folder,i in zip(folders,range(len(folders))):
        ...
        max_x_range = max(max_x_range,tissue_positions_list["x"].max())*1.01
        max_y_range = max(max_y_range,tissue_positions_list["y"].max())*1.01
        max_col_range = max(max_col_dims,tissue_positions_list["tissue_col"].max())*1.01
        max_row_range = max(max_row_dims,tissue_positions_list["tissue_row"].max())*1.01
