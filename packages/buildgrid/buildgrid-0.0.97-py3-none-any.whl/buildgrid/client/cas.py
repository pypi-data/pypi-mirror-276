...
class Downloader:
    ...
    def _fetch_blob(self, digest: Digest) -> bytearray:
        ...
        for read_response in self.__bytestream_stub.Read(read_request, metadata=context_module.metadata_list()):
    ...
    def _fetch_blob_batch(
        ...
        if not _CallCache.unimplemented(self.channel, "BatchReadBlobs"):
            ...
            try:
                batch_response = self.__cas_stub.BatchReadBlobs(batch_request, metadata=context_module.metadata_list())
...
class Uploader:
    ...
    def _send_blob(self, blob: BinaryIO, digest: Optional[Digest] = None) -> Digest:
        ...
        write_response = self.__bytestream_stub.Write(write_requests, metadata=context_module.metadata_list())
        ...
        if not _CallCache.unimplemented(self.channel, "BatchUpdateBlobs"):
            ...
            try:
                batch_response = self.__cas_stub.BatchUpdateBlobs(
                    batch_request, metadata=context_module.metadata_list()
                )
