...
class SQLIndex(IndexABC):
    ...
    def delete_n_bytes(
        ...
        with self._sql.scoped_session(exceptions_to_not_rollback_on=[StaleDataError]) as session:
            ...
            if self._delete_premarked_blobs:
                ...
                if not dry_run:
                    publish_counter_metric(CLEANUP_INDEX_PREMARKED_BLOBS_METRIC_NAME, len(marked_digests), metadata)
            ...
            if collected_bytes < n_bytes:
                ...
                if not dry_run:
                    publish_timer_metric(CLEANUP_INDEX_MARK_DELETED_METRIC_NAME, mark_deleted_duration, metadata)
                LOGGER.info(f"Gathered {collected_bytes} out of {n_bytes} bytes(max)")

        if dry_run:
            return collected_bytes

        with DurationMetric(CLEANUP_STORAGE_BULK_DELETE_METRIC_NAME, instanced=True):
            failed_deletes = self._storage.bulk_delete(storage_digests)

        publish_counter_metric(CLEANUP_STORAGE_DELETION_FAILURES_METRIC_NAME, len(failed_deletes), metadata)
        digests_to_delete = [x for x in marked_digests if x.hash not in failed_deletes]

        with self._sql.scoped_session(exceptions_to_not_rollback_on=[StaleDataError]) as session:
            with DurationMetric(CLEANUP_INDEX_BULK_DELETE_METRIC_NAME, instanced=True):
    ...
    def bulk_delete(self, digests: List[Digest]) -> List[str]:
        ...
        digests_to_delete = [x for x in digests if x.hash not in failed_deletes]
