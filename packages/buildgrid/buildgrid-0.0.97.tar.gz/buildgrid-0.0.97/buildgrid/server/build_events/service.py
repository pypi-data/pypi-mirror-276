...
from buildgrid.server.metrics_names import (
    BUILD_EVENTS_PUBLISH_BUILD_TOOL_EVENT_STREAM_EXCEPTION_COUNT_METRIC_NAME,
    BUILD_EVENTS_PUBLISH_LIFE_CYCLE_EVENT_EXCEPTION_COUNT_METRIC_NAME,
    BUILD_EVENTS_QUERY_EVENT_STREAMS_EXCEPTION_COUNT_METRIC_NAME,
)
from buildgrid.server.metrics_utils import ExceptionCounter, generator_method_exception_counter
...
class PublishBuildEventService(PublishBuildEventServicer, InstancedServicer[BuildEventStreamStorage]):
    ...
    FULL_NAME = PBE_DESCRIPTOR.services_by_name["PublishBuildEvent"].full_name

    @ExceptionCounter(BUILD_EVENTS_PUBLISH_LIFE_CYCLE_EVENT_EXCEPTION_COUNT_METRIC_NAME)
    def PublishLifecycleEvent(self, request: PublishLifecycleEventRequest, context: grpc.ServicerContext) -> Empty:
        ...
        """
        LOGGER.info("Received a PublishLifecycleEvent request")
    ...
    @track_request_id
    @generator_method_exception_counter(BUILD_EVENTS_PUBLISH_BUILD_TOOL_EVENT_STREAM_EXCEPTION_COUNT_METRIC_NAME)
    def PublishBuildToolEventStream(
        ...
        LOGGER.info("Opened a BuildToolEvent stream")
...
class QueryBuildEventsService(QueryBuildEventsServicer, InstancedServicer[BuildEventStreamStorage]):
    ...
    FULL_NAME = QBE_DESCRIPTOR.services_by_name["QueryBuildEvents"].full_name

    @track_request_id
    @ExceptionCounter(BUILD_EVENTS_QUERY_EVENT_STREAMS_EXCEPTION_COUNT_METRIC_NAME)
    def QueryEventStreams(
        self, request: QueryEventStreamsRequest, context: grpc.ServicerContext
    ) -> QueryEventStreamsResponse:
        LOGGER.info("Got a QueryEventStreams request")
