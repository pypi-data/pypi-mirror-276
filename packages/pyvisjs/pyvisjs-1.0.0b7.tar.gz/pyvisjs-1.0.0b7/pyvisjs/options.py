    ...
    class PyvisjsExtra(BaseDictable):
        ...
        def set_dataTable(self, position:str=None, columns=None, data=None):
            ...
            if data == None:
                ...
                if columns == None:
                    columns = ["from", "to"]
            elif isinstance(data, list) and isinstance(data[0], dict) and columns == None:
        ...
        def to_dict(self, nodes:List=None, edges:List=None) -> Dict:
            result = {}

            # ---------- filtering -----------
            result["filtering_enabled"] = len(self.filtering) > 0
            ...
            if "edge_filtering" in self.filtering:
                ...
                for field_name in edge_filtering:
                    ...
                    unique_values = list(set([str(getattr(edge, field_name)) for edge in edges if hasattr(edge, field_name)]))
                    unique_values.sort()
                    edge_filtering_lookup[tp_field_name] = unique_values

                result["edge_filtering_lookup"] = edge_filtering_lookup
            ...
            if "node_filtering" in self.filtering:
                ...
                for field_name in node_filtering:
                    unique_values = list(set([str(getattr(node, field_name)) for node in nodes if hasattr(node, field_name)]))
                    unique_values.sort()
                    node_filtering_lookup[field_name] = unique_values

                result["node_filtering_lookup"] = node_filtering_lookup 
            ...
            # ---------- startAnimation -----------
            result["startAnimation_enabled"] = len(self.startAnimation) > 0
            
            if "zoom_factor" in self.startAnimation:
                result["zoom_factor"] = self.startAnimation["zoom_factor"]

            if "duration_ms" in self.startAnimation:
                result["duration_ms"] = self.startAnimation["duration_ms"]

            # ---------- dataTables -----------
            result["dataTables_enabled"] = len(self.dataTables) > 0

            #if "columns" in self.dataTables:
            #    result["columns"] = self.dataTables["columns"]

            #if "tables" in self.dataTables:
            #    result["tables"] = self.dataTables["tables"]
