...
def list_file_download_url(file_id: str, drive_id: str = None):
    ...
    if file.type == 'folder':
        for i in ali.get_file_list(parent_file_id=file_id, drive_id=drive_id):
            print(i.name, i.download_url or i.url or ali.get_download_url(file_id=i.file_id, drive_id=i.drive_id).url)
    else:
        print(file.name,
              file.download_url or file.url or ali.get_download_url(file_id=file.file_id, drive_id=file.drive_id).url)
    return 'Done!'
