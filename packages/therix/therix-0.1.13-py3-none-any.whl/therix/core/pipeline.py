    ...
    async def async_invoke(self, question=None, session_id=None , keyword_search_params=None, dynamic_system_prompt=None):
        ...
        if pipeline_type == PipelineTypeMaster.RAG.value:
            ...
            if keyword_search_params and question is None:
                ...
                answer = await self.pipeline_service.async_search_keywords(keyword_search_params)
            else:
                answer = await self.pipeline_service.async_invoke_pipeline(
                    self.pipeline_data.id, question, session_id, trace_details, pipeline_system_prompt
                ) if pipeline_system_prompt else await self.pipeline_service.invoke_pipeline(
        ...
        # Handle Summarizer pipeline type
            ...
            answer = await self.pipeline_service.async_invoke_summarizer_pipeline(
                self.pipeline_data.id, text=question, trace_details=trace_details, system_prompt=pipeline_system_prompt
            ) if pipeline_system_prompt else await self.pipeline_service.async_invoke_summarizer_pipeline(
