...
# instead of the hardcoded line bellow
...
class GoogleContacts(GenericCRUD, metaclass=MetaLogger,
    ...
    # TODO Shall you write a small Python file to reconnect to Google Account using the refresh token in
    ...
    def authenticate(self, email: str):
        ...
        if not self.creds or not self.creds.valid:
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                refresh_token = self.select_one_value_by_column_and_value(
                    schema_name="user_external", view_table_name="user_external_view", select_clause_value="refresh_token",
                    column_name="username", column_value=email,
    ...
    def __insert_organization_profile(self, organization_id: int, profile_id: int) -> Optional[int]:
    ...
    @staticmethod
    def __insert_link_contact_groups(contact_dict: dict, contact_id: int) -> list:
    ...
    @staticmethod
    def __insert_link_contact_persons(contact_dict: dict, contact_id: int) -> dict:
    ...
    @staticmethod
    def __insert_link_contact_email_addresses(contact_dict: dict, contact_id: int, person_id: int = None,
        ...
        if email1:
            contact_email_addresses.insert_contact_and_link_to_email_address(
                contact_dict=contact_dict, contact_email_address_str=email1, contact_id=contact_id,
                person_id=person_id, profile_id=profile_id)
        if email2:
            contact_email_addresses.insert_contact_and_link_to_email_address(
                contact_dict=contact_dict,
    ...
    def __insert_link_contact_notes_and_text_blocks(
            contact_dict: dict, contact_id: int, profile_id: int) -> Optional[int]:
    ...
    @staticmethod
    def __insert_link_contact_phones(contact_dict: dict, contact_id: int, profile_id: int, person_id: int, 
                                     location_id: int, country_id: int) -> None:
        ...
        if phone1:
            contact_phones.insert_contact_and_link_to_existing_or_new_phone(
                contact_dict=contact_dict, phone_number=phone1, contact_id=contact_id,
                profile_id=profile_id, person_id=person_id, location_id=location_id, country_id=country_id)
        if phone2:
            contact_phones.insert_contact_and_link_to_existing_or_new_phone(
                contact_dict=contact_dict, phone_number=phone2, contact_id=contact_id,
                profile_id=profile_id, person_id=person_id, location_id=location_id, country_id=country_id)
        if phone3:
            contact_phones.insert_contact_and_link_to_existing_or_new_phone(
                contact_dict=contact_dict, phone_number=phone3, contact_id=contact_id,
                profile_id=profile_id, person_id=person_id, location_id=location_id, country_id=country_id)
        
    @staticmethod
    def __insert_link_contact_user_external(contact_dict: dict, contact_id: int) -> int:
    ...
    @staticmethod
    def __insert_contact_profiles(contact_dict: dict, contact_id: int, person_id: int) -> dict:
    ...
    @staticmethod
    def __insert_link_contact_domains(contact_dict: dict, contact_id: int, profile_id: int) -> list[dict]:
    ...
    def __insert_link_contact_location(self, contact_dict: dict, contact_id: int) -> Optional[list[dict]]:
