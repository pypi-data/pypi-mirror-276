...
from schwabkit import terminal
...
class Client:
    ...
    def __init__(self, app_key, app_secret, callback_url="https://127.0.0.1", tokens_file="tokens.json"):
        ...
        atIssued, rtIssued, tokenDictionary = self._read_tokens_file()
        if None not in [atIssued, rtIssued, tokenDictionary]:
            # show user when tokens were last updated and when they will expire
            self.access_token = tokenDictionary.get("access_token")
            self.refresh_token = tokenDictionary.get("refresh_token")
            self.id_token = tokenDictionary.get("id_token")
            self._access_token_issued = atIssued
            self._refresh_token_issued = rtIssued
