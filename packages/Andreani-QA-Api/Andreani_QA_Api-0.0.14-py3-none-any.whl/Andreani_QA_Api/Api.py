...
class Api(Functions, Parameters):
    ...
    def set_new_value_json(self, json_data, claves, valor_nuevo=None):
        ...
        if len(claves) == 1:
            ...
            if isinstance(json_data, list):
                Api.set_new_value_json(self, json_data[0][claves[0]], claves[1:], valor_nuevo)
            else:
                Api.set_new_value_json(self, json_data[claves[0]], claves[1:], valor_nuevo)
        return json_data
    ...
    def delete_value_json(self, json_data, claves):
        ...
        if len(claves) == 1:
            ...
            if isinstance(json_data, list):
                Api.delete_value_json(self, json_data[0][claves[0]], claves[1:])
            else:
                Api.delete_value_json(self, json_data[claves[0]], claves[1:])
        return json_data
