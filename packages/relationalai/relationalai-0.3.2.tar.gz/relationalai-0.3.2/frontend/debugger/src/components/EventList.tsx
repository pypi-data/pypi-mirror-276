...
export function EventListItem(props: EventItemProps) {
    const component = (() => {
        ...
        if (Message.is_error(event)) return EventItemError;
...
export function EventLeaf(props: EventLeafProps<Subject>) {
    ...
    const klass = () => classy("event-list-item leaf", props.event.event, props.event.span_type, props.class, { running: running(), selected: is_selected() });
    const style = () => {
        return {
            "--node-height": `${2 * radius()}px`
        }
...
export function EventItemError(props: EventItemProps<Message.Error>) {
    return (
        <EventLeaf {...props}>
            <div class="event-after">
                <span class="event-detail">
