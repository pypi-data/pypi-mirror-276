...
class Resources(ResourceProvider):
    ...
    def _exec_async_v1(
        ...
        with debugging.span("transaction") as txn_span:
            ...
            txn_id, state = next(iter(response))
    ...
    def get_transaction(self, transaction_id):
        results = self._exec(f"CALL {APP_NAME}.api.get_transaction(?);", [transaction_id])
        if not results:
            return None
        txn = self.txns_to_dicts(results)[0]
    ...
    def list_transactions(self, **kwargs):
        ...
        if not results:
            return []
        return self.txns_to_dicts(results)
