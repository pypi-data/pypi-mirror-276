from .errors import RAIInvalidVersionError, InvalidVersionKind
...
#
def check_dependencies(response, platform):
    try:
    ...
    except RAIInvalidVersionError as e:
        warnings.warn(f"[yellow]{e.message}")
...
def _compare_dependencies(expected, lock, std, platform="snowflake"):

    # database is pre-versioning
    if not lock and not std:
        raise RAIInvalidVersionError(InvalidVersionKind.SchemaOutOfDate, expected, lock, platform)

    # database has std but no lock, it is using a package manager
    if not lock and std:
        raise RAIInvalidVersionError(InvalidVersionKind.LibraryOutOfDate, expected, lock, platform)
    ...
    if errors:
        ...
        raise RAIInvalidVersionError(kind, expected, lock, platform, errors)
