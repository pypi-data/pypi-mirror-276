...
class PromptManagerClient:
    ...
    def create_prompt(
        ...
        try:
            self._grpc_client.CreatePrompt(request)
        except grpc.RpcError as error:
            call: grpc.Call = error  # noqa
            if call.code() == grpc.StatusCode.ALREADY_EXISTS:
                raise QwakException(f"Prompt with name: {name} already exist!")
    ...
    def delete_prompt_version(self, name: str, version: int):
        try:
            ...
            if call.code() == grpc.StatusCode.NOT_FOUND:
                raise QwakException(
                    f"There is no prompt named '{name}' or version number '{version}' that can be deleted"
                )
    ...
    def get_prompt_by_name(self, name: str) -> ProtoPrompt:
        ...
        try:
            ...
            call: grpc.Call = error  # noqa
            if call.code() == grpc.StatusCode.NOT_FOUND:
                raise QwakException(f"There is no prompt named '{name}'")
            else:
                raise QwakException(f"Internal Error: {call.details()}")
    ...
    def get_prompt_version_by_name(self, name: str, version: int) -> ProtoPromptVersion:
        ...
        try:
            ...
            if call.code() == grpc.StatusCode.NOT_FOUND:
                raise QwakException(
                    f"There is no prompt named '{name}' or version number '{version}'"
                )
            else:
                raise QwakException(f"Internal Error: {call.details()}")
