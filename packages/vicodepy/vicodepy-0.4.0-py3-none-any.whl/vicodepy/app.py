...
class Player(QMainWindow):
    ...
    def open_file(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, dialog_txt, None, "Video Files (*.mp4)"
        )
        if not filename:
            return
...
class ZoomableGraphicsView(QGraphicsView):
    ...
    def zoom_out(self):
        if self.zoomFactor / 1.2 >= self.minimum_zoomFactor:
            self.zoomFactor /= 1.2
            self.update_scale()
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Escape:
            ...
            if self.currentAnnotation is not None:
                ...
                if confirm_box.result() == AnnotationConfirmMessageBox.DialogCode.Accepted:
                    self.currentAnnotation.remove()
                    self.update()
...
class TimelineScale(QGraphicsRectItem):
    ...
    def _draw_scale(self, painter):
        step = self.timeline.view.width() / self.nb_ticks
        y = self.y() + self.rect().height()
        j = 0.0
        for i in np.arange(j, self.timeline.scene.width(), step):
            line_height = 10
            # Calculate the time of the ticks based on timeline scene width and duration
            if j % 2 == 0:
                time = i * self.timeline.duration / self.timeline.scene.width()

                # Calculate the size of the text
                font_height = painter.fontMetrics().height()

                # Calculate the position of the text
                text_position = QPointF(i, font_height)

                # Draw the text
                painter.drawText(
                    text_position, milliseconds_to_formatted_string(int(time))
                )
            else:
                line_height = 5
...
class AnnotationHandle(QGraphicsRectItem):
    def __init__(self, annotation: Annotation, value: int, x: float):
        ...
        width = 9
        height = annotation.rect().height() / 2
        self.setRect(QRectF(-4.5, 0, width, height))
...
class AnnotationStartHandle(AnnotationHandle):
    ...
    def change_time(self, time):
        ...
        self.annotation.startTime = time
...
class AnnotationEndHandle(AnnotationHandle):
    ...
    def paint(self, painter, option, widget=...):
        self.setX(self.annotation.rect().width())
        super().paint(painter, option, widget)
