...
def config_to_docker(config_path: pathlib.Path, config: Config):
    ...
    working_dir = None
    pip_install = "pip install -c constraints.txt"
    ...
    faux_pkgs_str = f"{os.linesep}{os.linesep}".join(
        f"ADD {relpath} /deps/{fullpath.name}/{fullpath.name}{os.linesep}                RUN touch /deps/{fullpath.name}/pyproject.toml"
        + (
