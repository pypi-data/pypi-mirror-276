...
from src.config import DEFAULT_ITERATIONS
from typing import Union

def get_key(password: bytes, salt: bytes, iterations: int) -> bytes:
  """Generate encryption key from password, salt, and number of iterations."""
  kdf = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32, salt=salt, iterations=iterations)
  return base64.urlsafe_b64encode(kdf.derive(password))


def encrypt_message(msg: str, password: str, iterations: int = DEFAULT_ITERATIONS) -> bytes:
    """Encrypts a message using a password."""
    msg_bytes = msg.encode('utf-8')
    salt = secrets.token_bytes(32)
    key = generate_encryption_key(password.encode('utf-8'), salt, iterations)
    encrypted_msg = Fernet(key).encrypt(msg_bytes)
