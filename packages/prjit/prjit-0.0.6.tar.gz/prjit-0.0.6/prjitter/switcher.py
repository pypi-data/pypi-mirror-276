...
class PrjitSwitcher:
    ...
    def plan_switch_in(self, name: str, kill_processes: bool=True):
        switch_out_commands = self.plan_switch_out(kill_processes)
        switch_in_commands = self.plan_mount_paths(name)
        return switch_out_commands + switch_in_commands

    def plan_switch_out(self, kill_processes: bool=True) -> list[list[str]]:
