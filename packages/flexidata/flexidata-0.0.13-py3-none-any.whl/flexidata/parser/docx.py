...
class DocxParser(FileHandler):
    ...
    def _parse_table(self, table: Table):
        """
        Parses a given table from the document and processes it into both HTML and plain text formats,
        applying different methods based on the `use_explicit_spans` flag setting.

        Parameters:
            table (Table): The table object to parse.

        Description:
            Depending on the `use_explicit_spans` attribute of the class, this method selects between two different parsing modes:
            1. With explicit spans: It uses methods that account for rowspan and colspan explicitly.
            2. Without explicit spans: It uses a simpler parsing method that ignores spans.
            After parsing, the method generates a TextBlock object that includes metadata such as file path,
            language, file type, and page number. The generated TextBlock is then added to the document's text blocks.

        Processes:
            - Converts the table to HTML and text formats.
            - Constructs metadata for the table.
            - Creates a TextBlock instance with the table data and metadata.
            - Appends the TextBlock to the document's list of text blocks.
            - Calls `_process_inner_table` to handle any nested tables or additional processing.
        """
        if self.use_explicit_spans:
            html_table = self.convert_table_to_text_with_span(table, as_html=True)
            text_table = self.convert_table_to_text_with_span(table, as_html=False)
        else:
        ...
        self._process_inner_table(table)
    ...
    def _parse_paragraph(self, paragraph: Paragraph):
        ...
        if self.extract_image:
            image_texts = self._process_image(paragraph)
    ...
    def _process_inner_table(self, table: Any) -> None:
        """
        ...
        Example:
            >>> _process_inner_table(table_instance)
            ...
            for cell in row.cells:
                for item in cell.iter_inner_content():
                    if type(item).__name__ == "Table":
                        ...
                        self.text_blocks.append(text_block)
                        self._process_inner_table(item)
                    else:
                        if self.extract_image:
                            image_texts = self._process_image(item)
                            self.text_blocks.extend(image_texts)

    def _process_image(self, paragraph: Any) -> List[TextBlock]:
        """
