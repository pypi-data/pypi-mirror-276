...
ComputeArgs = Union[DatasetDatum, Dict[str, Any]]
...
class MetricsClient:
    ...
    def compute(
        self, metric: Metric, args: Union[ComputeArgs, List[ComputeArgs]], **kwargs
    ):
        ...
        if isinstance(args, list):
            return self._batch_compute(metric.value, args, **kwargs)
        else:
            return self._compute(metric.value, args, **kwargs)

    def _compute(self, metric_name: str, args: ComputeArgs, **kwargs):
    ...
    def _batch_compute(
        ...
        **kwargs,
        ...
        if "LLMBased" in metric_name and self._client.timeout < 4 * num_datums:
            raise ValueError(
