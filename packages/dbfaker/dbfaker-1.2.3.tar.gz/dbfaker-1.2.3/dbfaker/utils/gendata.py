    ...
    def _import_package(self):
        ...
        for class_name in dir(self.faker):
            ...
            if callable(provider) and not class_name.startswith('_'):
                self.all_package[class_name] = provider

        self.all_package['faker'] = self.faker
            ...
            for key, value in copy.deepcopy(kwargs).items():
                try:
                    ...
                    if not isinstance(value, dict):
                        _data = {key: self._gen_field(**{"engine": "eq", "rule": {"value": _value}})}
    ...
    def _gen_field(self, **kwargs):
        ...
        if "(" in engine and ")" in engine:
            ...
            if isinstance(rule, list):
                r = eval("{engine}(*{rule})".format(engine=engine, rule=rule), self.env_data)
            elif isinstance(rule, dict):
                r = eval("{engine}(**{rule})".format(engine=engine, rule=rule), self.env_data)
            elif rule is None:
                r = eval("{engine}()".format(engine=engine), self.env_data)
            else:
