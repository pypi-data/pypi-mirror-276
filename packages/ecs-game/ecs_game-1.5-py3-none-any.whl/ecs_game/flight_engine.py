...
def render_game(screen, background, rocket, width):
    font = f.Font(None, 21)
    screen.clear()
    screen.blit(background, [0, 0])
    score_string = "X: " + str(rocket.x)+ "    Y: " + str(rocket.y)
    rocket.draw()
...
def update_cycle(rocket, is_onscreen):
    commanded_pos = list(rocket.pos)
    if k.up:
        commanded_pos[1] -= 10
    if k.down:
        commanded_pos[1] += 10
    ...
    if is_onscreen(commanded_pos[0], commanded_pos[1]):
        rocket.pos = commanded_pos
    else:
