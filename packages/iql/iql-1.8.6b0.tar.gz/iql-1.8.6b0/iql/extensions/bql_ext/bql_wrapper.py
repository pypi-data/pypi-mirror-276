...
def _execute_bql_str_list_async_orig(
    ...
    from . import (
        bql_service,
    )  # the import is pushed down so that bql isn't loaded if not used

    with bql_service.bqllock:
        query_strings = [query.to_bql_query() for query in queries]

        bqService = bql_service.get_bqservice()
...
def _process_single_item_response_arrow(response, results: Iterable[pa.Table]):  # type: ignore
...
def _process_response_arrow(response, results):  # type: ignore
    ...
    for res in response._sirs:
        assert hasattr(res, "_SingleItemResponse__result_dict")
