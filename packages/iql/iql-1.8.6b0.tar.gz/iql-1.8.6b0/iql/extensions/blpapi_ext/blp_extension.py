...
class BlpApiListFieldsExtension(IqlExtension):
    ...
    def executeimpl(self, sq: SubQuery) -> Optional[pa.Table]:
        ...
        if sq.get_query() == "All":
            ...
            fi = bal.getFieldInfo(sq.get_query(), search=bool(search))
...
class BlpApiRefDataExtension(IqlExtension):
    ...
    def executeimpl(self, sq: SubQuery) -> Optional[pa.Table]:
        ...
        rd = bal.get_refdata(
            fields=fields, securities=securities, parameters=params, historical=historical  # type: ignore
        )

        if len(rd) == 0:
            raise ValueError("Empty response for query %s", sq)
        else:
