...
def get_mktdata(fieldName, equity="IBM US Equity"):
...
def get_user_entitlements(uuid):
    ...
    session = getBlpApiSession(svc_type)
    ...
    response = awaitResponse(session)
...
def getFieldInfo(fieldname: Optional[str] = None, search: bool = False):
    """If fieldname is None, returns all available fields"""

    svcType = "//blp/apiflds"
    session = getBlpApiSession(svcType)
    fldSvc = session.getService(svcType)

    if search:
        request = fldSvc.createRequest("FieldSearchRequest")
        ...
        request = fldSvc.createRequest("FieldListRequest")
        if not fieldname:
            fieldname = "All"
        request.set("fieldType", fieldname)  # type: ignore
        request.set("returnFieldDocumentation", True)  # type: ignore
    else:  # Individual Field
        request = fldSvc.createRequest("FieldInfoRequest")
        request.append("id", fieldname)  # type: ignore
        request.set("returnFieldDocumentation", True)  # type: ignore

    session.sendRequest(request)
    data = awaitResponse(session)
