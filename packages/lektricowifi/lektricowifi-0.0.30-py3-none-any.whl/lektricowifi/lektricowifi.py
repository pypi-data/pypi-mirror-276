...
async def main():
    """Example of reading things from a Lektrico device."""
    
    # async with Device("192.168.1.10") as device: # this is 1p7k
    # async with Device("192.168.1.4") as device: # this is m2w
    async with Device("192.168.100.11") as device: # this is 1p7k
    # async with Device("192.168.100.201") as device: # this is m2w
    # async with Device("192.168.100.117") as device: # this is TRI
       
        # Current name
        info: Info = await device.device_info(Device.TYPE_1P7K)
        # info: Info = await device.device_info(Device.TYPE_EM)
        # info: Info = await device.device_info(Device.TYPE_3P22K) # this is TRI
        # print(info)
        print(info.charger_state) # for 1p7k
        # print(info.lb_mode) # for m2w
        # print(info.fw_version)
#         print(LBMode.GREEN.value)
        settings: Settings = await device.device_config()
        # print(settings)
        # print(settings.type)
        print(settings.serial_number)
#         print(settings.board_revision)
#         answer: bool = await device._request("charge.start", METH_POST)
#         print(answer)
#         answer: bool = await device._request("Device_id.Get", METH_GET)
#         answer: bool = await device._request_get("Device_id.Get")
#         answer: bool = await device.send_charge_stop()
        # answer: dict = await device.set_auth(False)
#         answer: bool = await device.set_led_max_brightness(10)
#         answer: bool = await device.set_dynamic_current(32)
#         answer: bool = await device.set_user_current(32)
#         answer: bool = await device.set_charger_locked(False)
        # answer: dict = await device.set_load_balancing_mode(1)
        # print("acesta e raspunsul:")
        # print(answer)
