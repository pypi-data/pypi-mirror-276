...
def find_baseline(absorbance, smooth_window):
    ...
    down, _ = find_peaks(-absorbance, prominence=(1, None), width=(0, 100))
...
def find_para(x, y, peaks, tailing_fronting='tailing', peak_width=0.05):
    ...
    for peak in peaks:
        ...
        params = [peak_area, retention, 0.01, 0.02 * damping]
        peak_params.extend(params)
        lower_bound = [0, retention - 0.1, 0, -0.03 if damping == -1 else 0.01]
        upper_bound = [np.inf, retention + 0.1, peak_width, -0.01 if damping == -1 else 0.03]
