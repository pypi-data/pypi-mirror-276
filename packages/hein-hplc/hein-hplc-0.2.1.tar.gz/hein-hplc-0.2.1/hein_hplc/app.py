...
from hein_hplc.peak_analysis_v2.peak_analysis_main import PeakAnalysis
from hein_hplc.peak_analysis_v2.utils import read_uv
    ...
    dbc.Row([
        dbc.Col(
            html.Div([
                ...
                dcc.Slider(
                    id="peak-width", min=1, max=10, step=1, value=5,
                ...
                dbc.Label("Peak shift tolerance in second"),
                dcc.Slider(
...
def update_widget(contents, filename):
    ...
    """
    if contents is None:
        return [], 0, 1, [0, 1], None, html.H5("No files uploaded!")
    else:
        ...
        if not df.empty:
            ...
            marks = {str(x): str(x) for x in range(int(x_range_max) + 1)}
            return options, x_range_min, x_range_max, x_range_value, marks, html.H5(f"Success, {filename}")
        else:
            return [], 0, 1, [0, 1], None, html.H5("File type not supported!")
...
def update_plot(peak_height, smooth_window, dropdown_value, tailing, area_in_second, range_of_interest, peak_width,
                smooth_derivative, use_derivative, show_derivative, dead_volume, contents, filename):
    """
    ...
    if dropdown_value in df.columns:
        ...
        fig = peak_analysis_tool.peak_deconvolution(column_name=dropdown_value, smooth_window=smooth_window,
                                                    peak_width=peak_width / 100, area_in_second=area_in_second,
...
def generate_csv(n_clicks, peak_shift,
                 peak_height, smooth_window, tailing, range_of_interest, smooth_derivative,
                 use_derivative,
                 dead_volume,
                 contents, filename):
