...
from .misc import resample_dic_df, resample_ut
...
class MergeDICandUT:
    ...
    def _perform_df_assertions(self):
        ...
        """
        #TODO add test
        assert set(self.DF_UT_REQ_COLS).issubset(self.df_ut.columns), f"Missing columns in UT Dataframe: {set(self.DF_UT_REQ_COLS) - set(self.df_ut.columns)}" 
        assert set(self.DF_DIC_REQ_COLS).issubset(self.df_dic_wt.columns), f"Missing columns in DIC Dataframe: {set(self.DF_DIC_REQ_COLS) - set(self.df_dic_wt.columns)}" 
    ...
    def resample_data(self, save_flag:bool=True):
