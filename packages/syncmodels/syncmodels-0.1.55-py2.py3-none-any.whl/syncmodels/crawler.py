...
class iSchema:
    ...
    def guess_schema(self, names, data, **kw):
        ...
        fields = [d_fields[key] for key in names]
...
class iSession(iContext, iSchema, iAuthenticator):
    ...
    def _schema(self, uri):
        return self.CACHE.get(uri["path"], {}).get(self.context[KIND_KEY])
    ...
    async def _get_schema(self, uri):
        kind = self.context[KIND_KEY]
        info = self.CACHE.setdefault(uri["path"], {})[kind] = (
            await self._inspect_schema(uri, kind)
        )
        return info
...
class iAgent:
    ...
    async def bootstrap(self):
        ...
        for func, args, kwargs in self._bootstrap():
            ...
            kind = kwargs.get(KIND_KEY, None)  #  must exists!
    ...
    def handle(self, stream, **context):
        ...
        for data in stream:
            if isinstance(data, dict) and self.NEED_KEYS.issubset(data):
                _stream.extend([_ for _ in data["result"]])
...
class iSQLSession(iSession):
    ...
    async def get_samples(self, kind, N=10):
        pass

    def _locate_data(self, data, rows=1, **kw) -> Dict:
        "try to locate where are some typical 3rd party system result"
        result = {}
        candidates = set(["cols"])
        keys = list(data.keys())
        for key in chain(
    ...
    async def _inspect_schema(self, uri, kind):
        ...
        # query = f"SHOW CREATE TABLE {kind}"

        # TODO: review this code
        N = 10
        samples = await self.get_samples(kind, N)

        query = f"SELECT * FROM {kind} LIMIT {N}"
        payload = {
        ...
        async with self as session:
            ...
            async with response:
                if 200 <= response.status < 300:
                    ...
                    info = self.RESULT_STRUCTURE = self._locate_data(
                        data, rows=N
                    )
    ...
    async def update_params(self, url, params, context):
        ...
        #         since = 0
        #         query = f"SELECT entity_ts, measurement, contador FROM raw_waterconsumption WHERE entity_ts > {since}"
