...
from spyctl.commands.create import cluster_policy
from spyctl.commands.create import container_policy
from spyctl.commands.create import container_ruleset
from spyctl.commands.create import linux_svc_policy
from spyctl import api, cli, elastic_search
...
def create_trace_suppression_policy(
    ...
    **selectors,
    ...
    """
    if trace_id:
        trace = elastic_search.search_for_trace_by_uid(trace_id, ctx)
        if not trace:
        ...
        t_sum = elastic_search.search_for_trace_summary_by_uid(
            summary_uid, ctx
        )
        if not t_sum:
            cli.err_exit(
