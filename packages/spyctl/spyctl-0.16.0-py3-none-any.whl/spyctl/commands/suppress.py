...
from spyctl import cli, elastic_search
...
def handle_suppress_trace_by_id(orig_id: str, include_users: bool):
    ...
    Returns:
        None
    """
    trace = elastic_search.search_for_trace_by_uid(orig_id)
    if not trace:
    ...
    t_sum = elastic_search.search_for_trace_summary_by_uid(summary_uid)
    if not t_sum:
