...
def DatePkr():
    ...
    while True:
        try:
            pass
            year=Prompt.__init2__(None,func=mkint,ptext=f"Year[Default:{date.today().year}]",helpText="year to be used in date returned; use y|yesterday for yesterday's date",data='year')
            if year in [None,]:
                return None
            elif year in ['y',]:
               return mkYesterday()
    ...
    while True:
        try:
            pass
            month=Prompt.__init2__(None,func=mkint,ptext=f"Month[0..12|Default:{date.today().month}]",helpText="month to be used in date returned; use y|yesterday for yesterday's date",data='month')
            if month in [None,]:
                return None
            elif month in ['y',]:
               return mkYesterday()
    ...
    while True:
        try:
            pass
            day=Prompt.__init2__(None,func=mkint,ptext=f"Day[{'..'.join([str(i) for i in calendar.monthrange(date.today().year,date.today().month)])}|Default:{date.today().day}]",helpText="day to be used in date returned; use y|yesterday for yesterday's date",data='day')
            if day in [None,]:
                return None
            elif day in ['y',]:
               return mkYesterday()
