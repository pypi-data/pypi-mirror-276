...
class PostInitCallerMetaClass(type):
...
AbstractPostInitCallerMetaClass = new_class(
    "AbstractPostInitCallerMetaClass",
    (ABCMeta, PostInitCallerMetaClass),
    {},
)


@delegates()
class FileSystemIterableSource(
    IterableSource, MultiProcessingMixIn, ABC, metaclass=AbstractPostInitCallerMetaClass  # type: ignore[misc] # Reason: Mypy has limited metaclass support # noqa: B950
):
    ...
    def __post_init__(self) -> None:
        """Post-initialization method for performing checks on the datasource."""
        self._perform_max_file_check()

    def _perform_max_file_check(self) -> None:
        ...
        Raises:
            IterableDataSourceError: If the number of files in the directory exceeds the
                maximum allowed.
        """
        if len(self.file_names) > BITFOUNT_MAX_NUMBER_OF_DATASOURCE_FILES:
            raise IterableDataSourceError(
                f"Too many files in the directory match the criteria. Found "
                f"{len(self.file_names)} files, but the maximum number of files "
    ...
    def file_names(self) -> List[str]:
        ...
        """
        return [i for i in self._file_names if i not in self.skipped_files]
