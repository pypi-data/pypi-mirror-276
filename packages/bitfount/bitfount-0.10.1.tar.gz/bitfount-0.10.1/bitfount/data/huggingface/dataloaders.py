...
class HuggingFaceBitfountDataLoader(_BaseHuggingFaceBitfountDataLoader):
    ...
    def __iter__(
        ...
        Iterator[List[_SingleOrMulti[Union[torch.Tensor, str]]]],
    ]:
...
_HFDataBatchWithKey = List[_HFDataEntryWithKey]
...
class HuggingFaceIterableBitfountDataLoader(BitfountDataLoader):
    ...
    def convert_input_target_key(
        batch: _HFDataBatchWithKey,
    ) -> List[_SingleOrMulti[Union[torch.Tensor, str]]]:
        """Convert the input and target to match the hugging face expected inputs_.
    ...
    def _with_key_iter(
        ...
        # In either case, this will return a type of _HFDataEntryWithKey in
        # its iteration
        def _type_correct_dataset_iterator() -> Iterator[_HFDataEntryWithKey]:
            for sample in self.dataset:
                yield cast(_HFDataEntryWithKey, sample)

        if self.shuffle:
            # If the dataset should be shuffled, we use a reservoir sampling method
            # to sample from a buffer of elements from the dataset.
            buffer_: _HFDataBatchWithKey = []
            for sample in _type_correct_dataset_iterator():
