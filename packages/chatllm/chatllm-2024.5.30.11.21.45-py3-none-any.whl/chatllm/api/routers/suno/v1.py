...
async def create_music(
    ...
    redis_client.set(f"task:{task_id}", song_ids, ex=3600 * 24 * 30)
...
async def get_music(
    ...
    async with httpx.AsyncClient(base_url="http://154.3.0.117:39955", follow_redirects=True) as client:
        for song_id in ids:
            ...
            clips[song_id] = clip
            status.add(clip.get("status"))

    content = {
        "code": 200,
        "data": {
            "task_id": task_id,
            "status": ",".join(status) + "d",
            "input": str(clip.get("metadata")),
            "clips": clips,
        },
        "message": "success"
    }
    if response.is_success: await appu('ppu-001', api_key=api_key)
    return JSONResponse(content=content)
