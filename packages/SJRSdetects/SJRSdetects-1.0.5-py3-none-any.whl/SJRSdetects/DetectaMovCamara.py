...
class DetectaMovCamara:
    ...
    def iniciarDeteccion(self, resultados):
        self.threadCamara = threading.Thread(target=self.detectarMovimiento, args=(resultados,))
        self.threadCamara.start()

    def detenerDeteccion(self):
        if self.threadCamara is not None:
            self.threadCamara.join()