...
def setup_rest_api_parameters_for_table(session, root, request):
    ...
    try:
        ...
        session.sql("alter session unset enable_snow_api_for_table").collect()
        root._can_use_rest_api = orig_setting
...
@pytest.fixture
def setup_enable_rest_api_with_long_running(root):
    @contextmanager
    def _setup(ref_class):
        # TODO : SNOW-1314080 improve root's manipulation
        original_can_use_rest_api = root._can_use_rest_api
        root._can_use_rest_api = True
        original_enable_long_running_polling = root._enable_long_running_polling
        root._enable_long_running_polling = True
        ...
        try:
            ...
            root._can_use_rest_api = original_can_use_rest_api
            root._enable_long_running_polling = original_enable_long_running_polling
