...
for i in range(len(u_filtered)):
    for j in range(i+1, len(u_filtered)):
        ...
        if distance < distance_u_v:
            count_A += 1
            if u_filtered[i] > 0 and v_filtered[i] > 0 and u_filtered[j] < 0 and v_filtered[j] < 0:
                count_A_and_B += 1
        if u_filtered[i] > 0 and v_filtered[i] > 0 and u_filtered[j] < 0 and v_filtered[j] < 0:
            count_B += 1
p_A = count_A / (len(u_filtered) * (len(u_filtered) - 1) / 2)
p_A_given_B = count_A_and_B / count_B if count_B > 0 else 0
print(f"Вероятность P(A): {p_A:.1f}")
print(f"Условная вероятность P(A|B): {p_A_given_B:.1f}")
