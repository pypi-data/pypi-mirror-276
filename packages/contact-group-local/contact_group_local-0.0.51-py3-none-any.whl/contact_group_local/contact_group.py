...
class ContactGroups(GenericMapping, metaclass=MetaLogger, object=CONTACT_GROUP_PYTHON_PACKAGE_CODE_LOGGER_OBJECT):
    ...
    def insert_link_contact_group_with_group_local(
            self, *, contact_id: int,
            groups_names: list, lang_code: LangCode = None, parent_group_id: int = None,
            is_interest: bool = None, image: str = None,
            mapping_data_dict: dict = None) -> list[dict]:

        groups_array = []
        for group_num, group in enumerate(groups_names):
            ...
            if not group_id:
                ...
                group_dict = ContactGroups.create_group_dict(
                    group_title=group,
    ...
    @staticmethod
    def create_group_dict(*, group_title: str, parent_group_id: int = None,
