...
class VXMiniQMTTDAPI(XtQuantTraderCallback):
    ...
    def on_stock_trade(self, data: XtTrade) -> None:
        ...
        vxsched.publish(
            "on_execution_report", data=execrpt, channel=self._account.account_id
        )
...
if __name__ == "__main__":
    mdapi = VXMiniQMTMDAPI()
    # print(mdapi.current("000001.SZ", "600000.SH"))
    trade_dates = mdapi.calendar(start_date="2005-01-01")
    print(len(trade_dates), trade_dates[0], trade_dates[-1])
    sector_list = xtdata.get_sector_list()
    for sector in sector_list:
        symbols = xtdata.get_stock_list_in_sector(sector)
        df = mdapi.history(symbols, "1d", "20210101")
        print(df)
