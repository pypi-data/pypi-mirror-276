...
import lumipy.provider.process_wrapper
...
class Factory:
    ...
    def start(self):
        ...
        self.process_wrapper = lumipy.provider.process_wrapper.ProcessWrapper(self.cmd)

        self.process_wrapper.start_process()
        time.sleep(2)
        while self.process_wrapper.starting:
            self.errored = self.process_wrapper.process.poll() is not None or self.process_wrapper.errored
            if self.errored:
                break
            sleep(0.1)

        if self.process_wrapper.expired_certificates:
            if self.process_wrapper.process is not None:
        ...
        if not self.errored:
            print("Process has finished starting.")
    ...
    def stop(self):
        ...
        """
        if self.process_wrapper.process.poll() is not None:
            # no-op
            return

        emph_print('\nStopping python provider factory')

        stop_process(self.process_wrapper.process)

        while self.process_wrapper.process.poll() is None:
            pass
