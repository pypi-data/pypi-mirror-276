...
from lumipy.provider.provider_sets import int_test, int_test_with_proxy
...
class TestPythonProviderIntegration(BaseIntTestWithAtlas):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.manager_with_proxy = lp.ProviderManager(*providers_set_with_proxy, port=5003, user=user, domain=domain, via_proxy=True, _dev_dll_path=dll_path, whitelist_me=True)
        cls.manager_with_proxy.start()

        interval = 10

        for i in range(0, 90, interval):
            print(f"Sleeping for {interval} seconds. Total elapsed time: {i} seconds.")
            sleep(interval)
    ...
    def test_pandas_provider_with_proxy(self):
        self._check_provider_attr_exists('pandas_titanic_with_proxy')

        pt = self.atlas.pandas_titanic_with_proxy()
...
Col8,float64,TestTable3,9,2
    ...
    def test_provider_metadata_base_variant_1_with_proxy(self):
        ...
        meta = self.atlas.test_pyprovider_variant1_with_proxy.meta

        self.assertEqual('test.pyprovider.variant1.proxy', meta.name.lower())
        self.assertEqual(2, len(meta.columns))
        self.assertEqual(9, len(meta.parameters))
        self.assertEqual(2, len(meta.table_parameters))
