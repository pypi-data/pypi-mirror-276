...
class TestPythonProviderIntegration(BaseIntTestWithAtlas):
    ...
    def setUpClass(cls) -> None:
        ...
        providers = int_test()
        cls.manager = lp.ProviderManager(*providers, user=user, domain=domain, whitelist_me=True)
        cls.manager.start()
        ...
        query = f'select count(*) from sys.registration where name in ({", ".join(provider_names)})'
        c = lm.get_client()

        while count != providers.count() and iters < 6:
            print(f'iteration {iters}. Found {count} providers, but expected {providers.count()}.')
            count = c.run(query).values[0, 0]
            iters += 1
            sleep(30)

        if count != providers.count():
            raise ValueError(f'Failed: found {count} providers, but expected {providers.count()}.')
