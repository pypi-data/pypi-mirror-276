...
class Metagenomics:
    ...
    def seqs_from_sra(self,accession:str,target_dir:str,container:str="None",**kwargs)-> tuple[str,dict]:
        ...
        if container=="None":
            ...
            fasterq_dump_script+=f"\nfasterq-dump {sra_file} -O {acc_folder} --split-files --tmp {acc_folder}"
            fasterq_dump_script+=f"\nrm {sra_file}"
            ...
            fasterq_dump_script=""
            sra_file=acc_folder/(accession+".sra")
            fasterq_dump_script+=f"docker run -v {target_dir}:{target_dir} {self.config.adtoolbox_docker} fasterq-dump {sra_file} -O {acc_folder} --split-files --tmp {acc_folder}\n"
            ...
            fasterq_dump_script=""
            sra_file=acc_folder/(accession+".sra")
            fasterq_dump_script+=f"singularity exec {self.config.adtoolbox_singularity} fasterq-dump {sra_file} -O {acc_folder} --split-files --tmp {acc_folder}\n"
