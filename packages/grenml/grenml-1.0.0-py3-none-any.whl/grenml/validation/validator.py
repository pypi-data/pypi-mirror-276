...
class TopologyValidator:
    ...
    def _validate_topology(self, topology: Topology, parent=None):
        ...
        A topology must have the following:
         ...
         - It must have a primary owner
         - If it has primary owner, it must be in its institution list
         - It must have the global institution in the institution list
        :param topology: The topology being evaluated
        :param parent: The Parent Topology. If this topology is the top
                       level, then the parent should be None.
        :return: A list of any issues in the data.
        """
        errors = []
        errors += self._validate_object_defaults(topology, parent)
        if topology.primary_owner is None:
            errors.append(
                'Topology {} ID: {} is missing a primary owner. One owner must be supplied for a \
                topology'.format(
                    topology.name, topology.id,
                )
            )
        if topology.primary_owner and topology.primary_owner not in topology.institutions:
            errors.append(
                'Topology {} ID: {} primary owner ID {} must be in the Topologies Institutions'
                .format(
    ...
    def _validate_node(self, node: Node, topology: Topology):
        ...
        A node must have the following:
         ...
         - All owners in the node must be IDs for an institution in the
           parent topologies institution list
    ...
    def _validate_link(self, link: Link, topology: Topology):
        ...
        A link must have the following:
         ...
         - All owners in the link must be IDs for an institution in the
           parent topologies institution list
        ...
        if grenml_object._parent != parent:
            errors.append(
                '{} {} parent does not match the topology it is in. {} != {}'.format(
                    grenml_object.__class__.__name__,
                    grenml_object.id,
                    grenml_object._parent,
                    parent
                )
            )
    ...
    def _validate_additional_properties(self, grenml_object: GRENMLObject):
        """
        Validate that the object additional properties meets the XSD
        standards.
    ...
    def _validate_location(self, location: Location, enforce_coordinates=True):
        ...
        try:
            ...
            errors.append(
                '{} {} Longitude must be a value that coordinates to a Floating point value'
                .format(
                    location.__class__.__name__, location.id,
                )
            )
        ...
        if location.unlocode and not isinstance(location.unlocode, str):
            errors.append(
                '{} {} UN/LOCODE must be a string value'.format(
                    location.__class__.__name__, location.id)
            )
    ...
    def _validate_lifetime(self, lifetime: Lifetime):
        """
        Validate that the object lifetime fields meets the XSD
        standards. The lifetime must have the following:
         - If a lifetime has an end value, it must have a start value
         - The lifetime's start time must be a string and match the ISO
           format, if it exists
         - The lifetime's end time must be a string and match the ISO
           format, if it exists
