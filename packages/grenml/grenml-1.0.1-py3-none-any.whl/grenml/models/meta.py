...
from grenml.exceptions import AttributeIdError, AttributeLongitudeError, MatchError
...
class GRENMLObject:
    ...
    def __init__(
        ...
        if not id:
            # We default to determinate id's if no id format is
            # specified
    ...
    def match(self, **kwargs):
        """
        Perform a look up of all keyword arguments and compare them to
        ...
        __in: Look up to see if the attribute value is in the argument.
            Examples:
                ...
                owners__in=['test_id_1', 'test_id_2', 'test_id_3'] If
                owners is a list, evaluate that owners is
                    a sublist of the argument provided
        __contains: Look up to see if the attribute value contains the
            argument
            Examples:
                id__contains='CAN' will check for any ID containing
                the string CAN
                owners__contains=['test_id_1', 'test_id_2'] If owners
                is a list, evaluate that the attribute contains all
                the values in the provided argument

        Example Uses:
        match(id__contains='CAN', name='TEST') will return True if the
        ID contains 'CAN' and the name is 'TEST'
        match(id__in=['CAN_1', 'TEST', 'TEST 2'] name__contains='TEST_')
        will return True if the ID is in the given list
            and the name contains the string 'TEST_' in it.
        match(owners__contains='CAN_1') will match if ID 'CAN_1'
        is in the owners list.
        :return: If all provided values given match, return True, else
            False.
        """
        for param, search_value in kwargs.items():
            ...
            if '__' in param:
                ...
                if option == 'contains':
                    if isinstance(search_value, Iterable) and not isinstance(search_value, str):
                    ...
                    raise MatchError(f'The option "{option}" is not a valid match extension')
            else:
                if isinstance(value, Iterable) and not isinstance(value, str):
    ...
    def __eq__(self, other):
        """
        Dynamically evaluate that all attributes in this object and the
        other object match or, if given a string, that
        the string matches the ID of the object.
        :param other: The object being equated to
        :return: If all attributes and their values between the this
            object and the other object match, or the given string
            matches the ID, then return true, else false.
        """
