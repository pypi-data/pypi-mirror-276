...
class DATETIME_DIRECTORY_TYPE(Enum):
    YEAR = rf"({EQUALS_PREFIX_REGEX})({YEAR_REGEX})"
    MONTH = rf"({EQUALS_PREFIX_REGEX})({MONTH_REGEX})"
    DAY = rf"({EQUALS_PREFIX_REGEX})({DAY_REGEX})"
    HOUR = rf"({EQUALS_PREFIX_REGEX})({HOUR_REGEX})"
    MINUTE = rf"{EQUALS_PREFIX_REGEX}({MINUTE_REGEX})"
    SECOND = rf"({EQUALS_PREFIX_REGEX})({SECOND_REGEX})"
    FULL_SECOND = rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_SECOND_REGEX}"
    FULL_MINUTE = rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_MINUTE_REGEX}"
    FULL_HOUR = rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_HOUR_REGEX}"
    FULL_DAY = rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_DAY_REGEX}"
    FULL_MONTH = rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_MONTH_REGEX}"
...
def _get_ymdhm_from_datetime_directory(
    ...
    if datetime_directory_type == DATETIME_DIRECTORY_TYPE.YEAR and (
        ...
        return True, int(matches.group(2)), None, None, None, None, None
    ...
    if datetime_directory_type == DATETIME_DIRECTORY_TYPE.MINUTE and (
        ...
        return True, year, month, day, hour, int(matches.group(2)), None
    if datetime_directory_type == DATETIME_DIRECTORY_TYPE.SECOND and (
        ...
        return True, year, month, day, hour, minute, int(matches.group(2))
