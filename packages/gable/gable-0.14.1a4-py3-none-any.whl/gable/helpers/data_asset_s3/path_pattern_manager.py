...
SECOND_REGEX = r"[0-5][0-9]"
EPOCH_REGEX = r"?<!\d)\d{10}(?!\d"
# Time-based
UUID_REGEX_V1 = r"([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-1[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})"
# Name-based using MD5 hashing
UUID_REGEX_V3 = r"([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-3[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})"
# Randomly generated
UUID_REGEX_V4 = r"([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})"
# Name-based using SHA-1 hashing
UUID_REGEX_V5 = r"([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-5[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})"

DATE_PART_DELIMITERS = r"[-_: ]{0,2}"
FULL_DATE_MONTH_REGEX = rf"({YEAR_REGEX}){DATE_PART_DELIMITERS}({MONTH_REGEX})"
FULL_DATE_DAY_REGEX = rf"{FULL_DATE_MONTH_REGEX}{DATE_PART_DELIMITERS}({DAY_REGEX})"
FULL_DATE_HOUR_REGEX = rf"{FULL_DATE_DAY_REGEX}{DATE_PART_DELIMITERS}({HOUR_REGEX})"
FULL_DATE_MINUTE_REGEX = (
    rf"{FULL_DATE_HOUR_REGEX}{DATE_PART_DELIMITERS}({MINUTE_REGEX})"
)
FULL_DATE_SECOND_REGEX = (
    rf"{FULL_DATE_MINUTE_REGEX}{DATE_PART_DELIMITERS}({SECOND_REGEX})"
)
FULL_DATE_REGEXES = [
    FULL_DATE_SECOND_REGEX,
    FULL_DATE_MINUTE_REGEX,
...
# similar to FULL_DATE_*_REGEX but with delimiters in groups so that they can be used in re.sub for pattern replacement in this class
FULL_DATE_MONTH_REGEX_MATCH_DELIMITER = (
    rf"({YEAR_REGEX})({DATE_PART_DELIMITERS})({MONTH_REGEX})"
)
FULL_DATE_DAY_REGEX_MATCH_DELIMITER = (
    rf"{FULL_DATE_MONTH_REGEX_MATCH_DELIMITER}({DATE_PART_DELIMITERS})({DAY_REGEX})"
)
FULL_DATE_HOUR_REGEX_MATCH_DELIMITER = (
    rf"{FULL_DATE_DAY_REGEX_MATCH_DELIMITER}({DATE_PART_DELIMITERS})({HOUR_REGEX})"
)
FULL_DATE_MINUTE_REGEX_MATCH_DELIMITER = (
    rf"{FULL_DATE_HOUR_REGEX_MATCH_DELIMITER}({DATE_PART_DELIMITERS})({MINUTE_REGEX})"
)
FULL_DATE_SECOND_REGEX_MATCH_DELIMITER = (
    rf"{FULL_DATE_MINUTE_REGEX_MATCH_DELIMITER}({DATE_PART_DELIMITERS})({SECOND_REGEX})"
)

EQUALS_PREFIX_REGEX = rf"{NO_PRECEDING_DIGIT}[a-zA-Z-_:]*=?"

DATE_PLACEHOLDER_TO_REGEX = {
    # \1 is a back references to any generic prefix that might be before the temporal part
    # \N>1 are back references for the delimiters so that the original delimiters are retained in the calls to re.sub
    r"\1{YYYY\3MM\5DD\7HH\9mm\11ss}": rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_SECOND_REGEX_MATCH_DELIMITER}",
    r"\1{YYYY}/\3{MM}/\5{DD}/\7{HH}/\9{mm}": rf"({EQUALS_PREFIX_REGEX})({YEAR_REGEX})/({EQUALS_PREFIX_REGEX})({MONTH_REGEX})/({EQUALS_PREFIX_REGEX})({DAY_REGEX})/({EQUALS_PREFIX_REGEX})({HOUR_REGEX})/({EQUALS_PREFIX_REGEX})({MINUTE_REGEX})",
    r"\1{YYYY\3MM\5DD\7HH\9mm}": rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_MINUTE_REGEX_MATCH_DELIMITER}",
    r"\1{YYYY}/\3{MM}/\5{DD}/\7{HH}": rf"({EQUALS_PREFIX_REGEX})({YEAR_REGEX})/({EQUALS_PREFIX_REGEX})({MONTH_REGEX})/({EQUALS_PREFIX_REGEX})({DAY_REGEX})/({EQUALS_PREFIX_REGEX})({HOUR_REGEX})",
    r"\1{YYYY\3MM\5DD\7HH}": rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_HOUR_REGEX_MATCH_DELIMITER}",
    r"\1{YYYY}/\3{MM}/\5{DD}": rf"({EQUALS_PREFIX_REGEX})({YEAR_REGEX})/({EQUALS_PREFIX_REGEX})({MONTH_REGEX})/({EQUALS_PREFIX_REGEX})({DAY_REGEX})",
    r"\1{YYYY\3MM\5DD}": rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_DAY_REGEX_MATCH_DELIMITER}",
    r"\1{YYYY}/\3{MM}": rf"({EQUALS_PREFIX_REGEX})({YEAR_REGEX})/({EQUALS_PREFIX_REGEX})({MONTH_REGEX})",
    r"\1{YYYY\3MM}": rf"({EQUALS_PREFIX_REGEX}){FULL_DATE_MONTH_REGEX_MATCH_DELIMITER}",
    r"\1{YYYY}": rf"({EQUALS_PREFIX_REGEX})({YEAR_REGEX})",
}
...
class PathPatternManager:
    ...
    def substitute_if_unix_timestamp(self, path: str) -> str:
        ...
        """
        match = re.search(rf"({EPOCH_PLACEHOLDER_TO_REGEX['{epoch}']})", path)
        if match:
            ...
            try:
                ...
                if dt:
                    ...
                    if (
                        ...
                        heapq.heappush(
                            self.pattern_to_actual_paths[transformed_path],
                            PathPatternManager.PrioritizedItem(dt, (unquoted_path, dt)),
                        )
                    else:
