...
from kozmo_ai.data_preparation.git import REMOTE_NAME, Git, api
...
from kozmo_ai.shared.strings import capitalize_remove_underscore_lower
...
class GitBranchResource(GenericResource):
    ...
    def collection(self, query, meta, user, **kwargs):
        ...
        if repository:
            ...
            if include_remote_branches:
                try:
                    git_manager.fetch()
                    kozmo_remote = git_manager.origin
                    arr += [
                        dict(name=ref.name)
                        for ref in kozmo_remote.refs
                    ]
                except Exception:
    ...
    def get_oauth_config(
        ...
        if access_token:
            ...
            # Default to kozmo user email if no email is returned from API
