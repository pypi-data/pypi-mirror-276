...
from kozmo_ai.data_preparation.models.block.dbt.constants import DBT_DIRECTORY_NAME
from kozmo_ai.data_preparation.models.block.dbt.dbt_adapter import DBTAdapter
from kozmo_ai.data_preparation.models.block.dbt.dbt_cli import DBTCli
from kozmo_ai.data_preparation.models.block.dbt.profiles import Profiles
from kozmo_ai.data_preparation.models.block.dbt.project import Project
from kozmo_ai.data_preparation.models.block.dbt.sources import Sources
from kozmo_ai.data_preparation.models.constants import BlockLanguage
from kozmo_ai.data_preparation.shared.utils import get_template_vars
from kozmo_ai.orchestration.constants import PIPELINE_RUN_KOZMO_VARIABLES_KEY
...
class DBTBlock(Block):
    ...
    def create(cls, *args, **kwargs) -> 'DBTBlock':
        ...
        from kozmo_ai.data_preparation.models.block.dbt.block_sql import DBTBlockSQL
        from kozmo_ai.data_preparation.models.block.dbt.block_yaml import DBTBlockYAML
    ...
    def base_project_path(self) -> Union[str, os.PathLike]:
        """
        Path to base dbt project of the kozmo repository
        ...
        return simplejson.dumps(
            {
                ...
                if PIPELINE_RUN_KOZMO_VARIABLES_KEY != k and (
                    type(v) is str or
                    type(v) is int or
                    type(v) is bool or
                    type(v) is float or
    ...
    def materialize_df(
        ...
        `"{default_database}"."{default_schema}"."kozmo_{pipeline_uuid}_{block_uuid}"`
        ...
        for (project_path, target) in targets:
            ...
            seed_path = Path(
                project_path,
                seed_dir_name,
                f'kozmo_{pipeline_uuid}',
                f'kozmo_{pipeline_uuid}_{block_uuid}.csv'
            )
            ...
            with Profiles(project_path, variables) as profiles:
                args = [
                    ...
                    '--target', target,
                    '--select', f'kozmo_{pipeline_uuid}_{block_uuid}',
    ...
    def update_sources(
        ...
        """
        Update the kozmo_sources.yml for each dbt project in the pipeline based on the pipeline
