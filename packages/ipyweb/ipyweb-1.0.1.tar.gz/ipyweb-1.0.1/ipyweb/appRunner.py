...
class appRunner(metaclass=singleton):
    ...
    def boot(self):
        try:
            ...
            ipywebException(f'An exception occurred during application [{self.appName}]  startup: {e}')
            time.sleep(3)
            sys.exit(0)
    ...
    def getAppRunner(self):
        ...
        try:
            ...
            ipywebException(f'The application startup module does not exist:[{appModulePath}]')
        except Exception as e:
            ipywebException(f'An exception occurred while retrieving the startup module:[{appModulePath}]')
