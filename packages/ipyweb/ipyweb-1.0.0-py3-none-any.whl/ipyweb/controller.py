    ...
    def invoke(self, spacename='', *args):
        ...
        try:
            ...
            if handleWindowExist != True and isTarget == False:
                return controllerEventer._error(f'窗口[{windowName}]不存在或未打开')
            ...
            if self._controllers.get(controllerName, ''):
                ...
                if callable(getattr(controllerInvoke, actionName, None)):
                    ...
                    return controllerEventer._error(
                        f'窗口[{windowName}]控制器[{controllerName}]方法[{actionName}]不存在')
            else:
                return controllerEventer._error(f'窗口[{windowName}]控制器[{controllerName}]不存在')
        except Exception as e:
            return controllerEventer._error(f'窗口[{windowName}]控制器[{controllerName}]发生异常:{e}')
