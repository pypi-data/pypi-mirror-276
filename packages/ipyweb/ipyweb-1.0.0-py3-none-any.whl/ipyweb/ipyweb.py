...
class ipyweb():
    ...
    @classmethod
    def setOnOpened(self, onOpened=None):
        sysIpyweb.setOnOpened(onOpened)
        return self
...
class sysIpyweb():
    ...
    def bootAppRunner(self):
        ...
        def onWindowsOpened(appName):
            ...
            print(
                f'基础数据加载耗时:{round(self.bootTime["ipywebBoots"]["bootBaserRun"], 3)}s,'
                f'启动预载服务(含应用预载)耗时:{round(self.bootTime["ipywebBoots"]["bootPreloaderRun"], 3)}s,'
                f'[{appName}]窗口启动耗时:{round(self.bootTime["bootAppRun"], 3)}s')
    ...
    def getAppRunerOnEvent(self):
        ...
        if hasattr(appRunnerCls, 'onOpened'): self.setOnOpened(appRunnerCls.onOpened)
        return self
