...
class dbIpyweb():
    ...
    def open(self, **kwargs):
        try:
            ...
            if self._driver not in ['sqlite', 'mysql', 'postgresql']:
                logger.console.error(f'不支持的数据库驱动')
                return self._database
            if len(self._driverParams) <= 0:
                logger.console.error(f'数据库配置信息不存在')
                return self._database
            ...
            logger.console.error(f'数据库初始化失败：{e}')
        return self._database
    ...
    def _setConfig(self, config={}):
        try:
            ...
            logger.console.error(f'数据库配置信息获取异常：{e}')
        if type(self._config) == dict and len(self._config) <= 0:
            logger.console.error(f'数据库配置信息获取失败')
        return self
    ...
    def _setConnect(self, connect=''):
        try:
            ...
            logger.console.error(f'数据库连接名获取异常：{e}')
        return self
    ...
    def _setDriver(self, dirver=''):
        try:
            ...
            logger.console.error(f'数据库驱动名获取异常：{e}')
    ...
    def _setDriverParams(self, driverParams={}):
        try:
            ...
            logger.console.error(f'数据库连接配置获取异常：{e}')
        return self
