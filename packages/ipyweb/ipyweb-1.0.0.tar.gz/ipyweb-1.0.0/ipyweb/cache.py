...
class cacheIpyweb(metaclass=singleton):
    ...
    def _open(self):
        try:
            if self._driver not in ['diskcache']:
                logger.console.error(f'暂不支持的缓存驱动')
                return self._diskcache
    ...
    def _setConfig(self, config={}):
        try:
            ...
            logger.console.error(f'缓存配置信息获取异常：{e}')
        if type(self._config) == dict and len(self._config) <= 0:
            logger.console.error(f'缓存配置信息获取失败')
        return self
    ...
    def _setConnect(self, connect=''):
        try:
            ...
            logger.console.error(f'缓存连接名获取异常：{e}')
        return self
    ...
    def _setDriver(self, dirver=''):
        try:
            ...
            logger.console.error(f'缓存驱动名获取异常：{e}')
    ...
    def _setDriverParams(self, driverParams={}):
        try:
            ...
            logger.console.error(f'缓存配置获取异常：{e}')
        return self
