...
class Frame(ctk.CTkFrame):
    ...
    def page(self):
        ...
        self.mainpages_dict[self.page_choise].pack(expand=True, fill="both")                        #display the default page
    ...
    def reload_page(self, name):
        if name in self.mainpages_dict:
            ...
            if self.page_choise == name:
                self.pages_dict[name].pack_forget() 
                self.pages_dict[name].tools_f.place_forget()    #placed inside the file 
                self.pages_dict[name] = self.mainpages_dict[name]
                self.pages_dict[name].pack(expand=True, fill="both")
                self.pages_dict[name].called_when_opened(k=1)   # used to apply some changes that can't be applied before the frame is displayed
            else:
                self.pages_dict[name] = self.mainpages_dict[name]
                self.page_switcher(name)
            ...
            if self.page_choise == splited_name[0]:
                self.pages_dict[splited_name[0]].pack_forget() 
                self.pages_dict[splited_name[0]].tools_f.place_forget()    #placed inside the file 
                self.pages_dict[splited_name[0]] = self.subpages_dict[name]
                self.pages_dict[splited_name[0]].pack(expand=True, fill="both")
                self.pages_dict[splited_name[0]].called_when_opened(k=1)   # used to apply some changes that can't be applied before the frame is displayed
            else:
                self.pages_dict[splited_name[0]] = self.subpages_dict[name]
