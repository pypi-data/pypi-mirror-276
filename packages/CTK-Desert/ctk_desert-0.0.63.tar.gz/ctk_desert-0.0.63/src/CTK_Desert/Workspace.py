...
class Workspace(Page_BM):
    def __init__(self):
        super().__init__(scrollable=True, start_func=self.on_start, pick_func=self.on_pick, update_func=self.on_update, leave_func=self.on_leave)
    ...
    def on_start(self):
        for page in [page for page in self.menu_page_frame.mainpages_dict if page not in ["Workspace", "Settings"]]:
            self.pages_tabs.tab(page, os.path.join(os.path.abspath(os.path.join(self.icons_path, os.pardir)), "preview_window.png"),
                                button_icon=os.path.join(self.icons_path, "icons8-right-arrow-64.png"),
                                button_command=lambda page_name = page: Chest.Switch_Page(page_name))
