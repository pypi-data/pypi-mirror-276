...
class IconUtil:
    ...
    def format_style(
        ...
        if style is None:
            ...
            if default_icon_style is not None:
        ...
        default_style = dtheme.iconstyles.get()
        ...
        if style.halign is None:
            style.halign = default_style.halign
        if style.valign is None:
            style.valign = default_style.valign

        # apply system default
        if style.style is None:
            style.style = IconStyleDefault.style
        if style.color is None:
            style.color = IconStyleDefault.color
        if style.alpha is None:
            style.alpha = IconStyleDefault.alpha
        if style.halign is None:
            style.halign = IconStyleDefault.haligh
        if style.valign is None:
            style.valign = IconStyleDefault.valign
...
class ImageUtil:
    ...
    def format_style(style: Union[ImageStyle, str, None]) -> ImageStyle:
        ...
        theme_default = dtheme.imagestyles.get()

        # apply theme_default
        if style.lstyle is None:
            style.lstyle = theme_default.lstyle
        if style.lcolor is None:
            style.lcolor = theme_default.lcolor
        if style.lwidth is None:
            style.lwidth = theme_default.lwidth
        if style.halign is None:
            style.halign = theme_default.halign
        if style.valign is None:
            style.valign = theme_default.valign

        # apply system default
        if style.lstyle is None:
            style.lstyle = ImageStyleDefault.lstyle
        if style.lcolor is None:
            style.lcolor = ImageStyleDefault.lcolor
        if style.lwidth is None:
            style.lwidth = ImageStyleDefault.lwidth
        if style.halign is None:
            style.halign = ImageStyleDefault.halign
        if style.valign is None:
            style.valign = ImageStyleDefault.valign
    ...
    def format_style(
        ...
        if style is None:
            ...
            if dtheme.linestyles.has(name=style):
                ...
                raise ValueError('Both dtheme.linestyles and dthme.linearrowstyles do not have "{name}".')
        ...
        if isinstance(style, LineStyle):
            theme_default = dtheme.linestyles.get()

            # apply theme_default (theme)
            if style.width is None:
                style.width = theme_default.width
            if style.color is None:
                style.color = theme_default.color
            if style.style is None:
                style.style = theme_default.style
            if style.alpha is None:
                style.alpha = theme_default.alpha

            # apply system default
            if style.width is None:
                style.width = LineStyleDefault.width
            if style.color is None:
                style.color = LineStyleDefault.color
            if style.style is None:
                style.style = LineStyleDefault.style
            if style.alpha is None:
                style.alpha = LineStyleDefault.alpha

            return style

        theme_default = dtheme.linearrowstyles.get()
        # apply theme_default (theme)
        if style.lstyle is None:
            style.lstyle = theme_default.lstyle
        if style.lwidth is None:
            style.lwidth = theme_default.lwidth
        if style.hstyle is None:
            style.hstyle = theme_default.hstyle
        if style.hscale is None:
            style.hscale = theme_default.hscale
        if style.color is None:
            style.color = theme_default.color
        if style.alpha is None:
            style.alpha = theme_default.alpha
        ...
        # apply system default
        if style.lstyle is None:
            style.lstyle = LineArrowStyleDefault.lstyle
        if style.lwidth is None:
            style.lwidth = LineArrowStyleDefault.lwidth
        if style.hstyle is None:
            style.hstyle = LineArrowStyleDefault.hstyle
        if style.hscale is None:
            style.hscale = LineArrowStyleDefault.hscale
        if style.color is None:
            style.color = LineArrowStyleDefault.color
        if style.alpha is None:
            style.alpha = LineArrowStyleDefault.alpha
...
class ShapeUtil:
    ...
    def format_styles(
        ...
        # theme default
        default_style: ShapeStyle = get_style()
        if style.halign is None:
            style.halign = default_style.halign
        if style.valign is None:
            style.valign = default_style.valign
        if style.lwidth is None:
            style.lwidth = default_style.lwidth
        if style.lcolor is None:
            style.lcolor = default_style.lcolor
        if style.lstyle is None:
            style.lstyle = default_style.lstyle
        if style.fcolor is None:
            style.fcolor = default_style.fcolor
        if style.alpha is None:
            style.alpha = default_style.alpha

        # system default
        if style.halign is None:
            style.halign = ShapeStyleDefault.halign
        if style.valign is None:
            style.valign = ShapeStyleDefault.valign
        if style.lwidth is None:
            style.lwidth = ShapeStyleDefault.lwidth
        if style.lcolor is None:
            style.lcolor = ShapeStyleDefault.lcolor
        if style.lstyle is None:
            style.lstyle = ShapeStyleDefault.lstyle
        if style.fcolor is None:
            style.fcolor = ShapeStyleDefault.fcolor
        if style.alpha is None:
            style.alpha = ShapeStyleDefault.alpha
        ...
        # theme default
        default_textstyle: ShapeTextStyle = get_textstyle()
        if textstyle.color is None:
            textstyle.color = default_textstyle.color
        if textstyle.size is None:
            textstyle.size = default_textstyle.size
        if textstyle.halign is None:
            textstyle.halign = default_textstyle.halign
        if textstyle.valign is None:
            textstyle.valign = default_textstyle.valign
        if textstyle.font is None:
            textstyle.font = default_textstyle.font
        if textstyle.alpha is None:
            textstyle.alpha = default_textstyle.alpha

        # system default
        if textstyle.color is None:
            textstyle.color = ShapeTextStyleDefault.color
        if textstyle.size is None:
            textstyle.size = ShapeTextStyleDefault.size
        if textstyle.halign is None:
            textstyle.halign = ShapeTextStyleDefault.halign
        if textstyle.valign is None:
            textstyle.valign = ShapeTextStyleDefault.valign
        if textstyle.font is None:
            textstyle.font = ShapeTextStyleDefault.font
        if textstyle.alpha is None:
            textstyle.alpha = ShapeTextStyleDefault.alpha
...
class TextUtil:
    ...
    def format_style(style: Union[TextStyle, str, None]) -> TextStyle:
        ...
        theme_default = dtheme.textstyles.get()

        if style.color is None:
            style.color = theme_default.color
        if style.size is None:
            style.size = theme_default.size
        if style.halign is None:
            style.halign = theme_default.halign
        if style.valign is None:
            style.valign = theme_default.valign
        if style.font is None:
            style.font = theme_default.font
        if style.alpha is None:
            style.alpha = theme_default.alpha
        if style.bgalpha is None:
            style.bgalpha = theme_default.bgalpha
        if style.bglcolor is None:
            style.bglcolor = theme_default.bglcolor
        if style.bglstyle is None:
            style.bglstyle = theme_default.bglstyle
        if style.bglwidth is None:
            style.bglwidth = theme_default.bglwidth
        if style.bgfcolor is None:
            style.bgfcolor = theme_default.bgfcolor

        if style.color is None:
            style.color = TextStyleDefault.color
        if style.size is None:
            style.size = TextStyleDefault.size
        if style.halign is None:
            style.halign = TextStyleDefault.halign
        if style.valign is None:
            style.valign = TextStyleDefault.valign
        if style.font is None:
            style.font = TextStyleDefault.font
        if style.alpha is None:
            style.alpha = TextStyleDefault.alpha
        if style.bgalpha is None:
            style.bgalpha = TextStyleDefault.bgalpha
        if style.bgfcolor is None:
            style.bgfcolor = TextStyleDefault.bgfcolor
        if style.bglcolor is not None or style.bglstyle is not None or style.bglwidth is not None:
            if style.bglcolor is None:
                style.bglcolor = TextStyleDefault.bglcolor
            if style.bglstyle is None:
                style.bglstyle = TextStyleDefault.bglstyle
            if style.bglwidth is None:
                style.bglwidth = TextStyleDefault.bglwidth
