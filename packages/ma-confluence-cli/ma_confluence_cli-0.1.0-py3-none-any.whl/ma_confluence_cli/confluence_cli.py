...
def insert_transmit_table(cli, csv_file, parent_page_id, title):
    csv_file = '/home/yue.shao/unimarketaccessconf/scripts/gen_transmit_feedsource_mapping/transmit_data/feedsource_transmit_cn_with_start_time.csv'
    df=pd.read_csv(csv_file)
    rows, columns = df.shape
    column_names = df.columns
    table = cli.generate_template_table(rows+1, columns)  # generate a 3x2 tamplate table
    type_list = []
    for i in range(columns):
        type_list.append('str')
    table.set_row(row=0, content_list=column_names, type_list=type_list)
    for index, row in df.iterrows():
        table.set_row(row=index+1, content_list=row,  type_list=type_list)  # image and file tpye will be thumbnails
    created_page_id = cli.create_or_get_page_by_title_under_page(parent_page_id=parent_page_id, title=title)
    cli.clear_page(created_page_id)
    cli.insert_table(table=table, page_id=created_page_id)
