...
class JsonValidator:
    def __init__(self, core_schema, other_schemas):
        ...
        self._registry = Registry().with_resources([(k, Resource(contents=v, specification=DRAFT202012)) for k, v in self._schema_dict.items()])
        self._validator = Draft202012Validator(self._core_schema, registry=self._registry)
...
class TS103120Validator (JsonValidator):
    def __init__ (self):
        ...
        self._request_fragment_validator = Draft202012Validator(request_fragment_schema, registry=self._registry)
        response_fragment_schema = { "$ref" : "ts_103120_Core_2019_10#/$defs/ResponsePayload" }  
        self._response_fragment_validator = Draft202012Validator(response_fragment_schema, registry=self._registry)
    ...
    def expand_request_response_exception (self, ex):
        if list(ex.schema_path) == ['properties', 'Payload', 'oneOf']:
            ...
            if 'RequestPayload' in self.instance_doc['Payload'].keys():
                ret_list = list(chain(*[self.expand_action_exception(x) for x in self._request_fragment_validator.iter_errors(self.instance_doc['Payload']['RequestPayload'])]))
                for r in ret_list:
                    r.path = ex.path + r.path
                return ret_list
            elif 'ResponsePayload' in self.instance_doc['Payload'].keys():
                ret_list = list(chain(*[self.expand_action_exception(x) for x in self._request_fragment_validator.iter_errors(self.instance_doc['Payload']['ResponsePayload'])]))
                for r in ret_list:
                    r.path = ex.path + r.path
                return ret_list
            else:
    ...
    def expand_action_exception (self, ex):
        ...
        v = Draft202012Validator({"$ref" : f"ts_103120_Core_2019_10#/$defs/{verb}{message}"}, registry=self._registry)
