...
def map_to_plot(
    struct, kind=None, colours=None, bands=None, spectrum_overlay=False, survey=None
):
    if not struct.data:
    ...
    if plot_success:
        if struct.kind == "lightcurve":
            if kind == "lightcurve":
                ...
                plot = lomb_scargle(struct.data).powspec_plot
                dimensions = Dimensions(height=1, width=1)
            elif kind == "phase":
                from ..Timeseries.lomb_scargle import lomb_scargle

                plot = lomb_scargle(struct.data).phasefold_plot
                dimensions = Dimensions(height=1, width=1)
            else:
