class LightcurveStruct(object):
    ...
    def plot(self, kind="lightcurve", colours=None, bands=None):
        from ..Plotting.plotmap import map_to_plot

        return map_to_plot(self, kind, colours, bands)
...
class ZtfQuery(GeneralQuery):
    ...
    def generate_data(self, response):
        ...
        if len(data) == 0:
            print("Note: {self.survey} query returned no data.")
            return None
...
class AtlasQuery(GeneralQuery):
        ...
        while not task_url:
            with requests.Session() as s:
                ...
                if r.status_code == 201:
                    ...
                    print(
                        f"{self.survey} login failed. Please ensure that you have provided valid credentials."
                    )
                    return None
    ...
    def format_data(self, data):
        ...
        # chi > 100
        bad_indices_chi = set([i for i, element in enumerate(chi_arr) if element > 100])
        # abs(flux) < 3
