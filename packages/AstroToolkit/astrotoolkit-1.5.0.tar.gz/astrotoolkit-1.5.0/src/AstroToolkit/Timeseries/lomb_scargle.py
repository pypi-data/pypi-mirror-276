...
def lomb_scargle(data):
    class timeseries_data(object):
        def __init__(self, time, mag, mag_err, power, frequency):
            self.time = time
            self.mag = mag
        ...
        def powspec_plot(self):
            ...
            plot.line(
                x=self.frequency,
                y=self.power,
                legend_label=f"Max Frequency: {round(max_freq.value,2)} 1/d {newline}Period: {round(period.value,2)}",
            )
    ...
    data_class = timeseries_data(time, mag, mag_err, power, freqs)
