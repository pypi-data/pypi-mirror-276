...
def fit_threshold_by_channel(data: pd.DataFrame, ch: int, func):
...
def fit_thresholds(data: pd.DataFrame, channels: list[int]) -> pd.DataFrame:
    """スレッショルド
    ...
    erfc = erfc_function
    for c in channels:
        _threshold, _, _ = fit_threshold_by_channel(data, ch=c, func=erfc)
