...
class Vaul:
    ...
    def _handler(self, event):
        request = get_request()
        if request is None:
            return self._response(500, 'Internal server error')

        path, method = request.path, request.method
        route_handler = self._get_route_handler(path, method, event.get('requestContext', {}).get('domainName'))

        if not route_handler:
            return self._response(404, f"Path or method not found: {path}, {method}")
    ...
    def _run_action(self, request, path, method):
        try:
            ...
            return self._response(400, {'error': 'Invalid request body', 'details': str(e)})
        except ValueError as e:
            return self._response(400, {'error': str(e), 'details': 'Check the path and method or JSON body format.'})
        except Exception as e:
            print(e)
            return self._response(500, {'error': str(e), 'details': 'Internal server error'})
    ...
    def _generate_openapi_schema(self, url: str):
        ...
        return {
            "openapi": "3.0.0",
            "info": {"title": "Vaul API", "version": "1.0.0"},
            "servers": [{"url": url}],
            "paths": openapi_paths,
        }
