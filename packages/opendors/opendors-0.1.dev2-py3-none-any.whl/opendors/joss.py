...
class JossRetriever(WorkflowRule):
    ...
    def run(self) -> None:
        ...
        while True:
            self.log.info(f"Getting JOSS papers from page {page}.")
    ...
    def _get_page(self, page: int) -> requests.Response:
        """
        ...
        """
        return requests.get(f"{URL_PUBLISHED_PAPERS}.json?page={page}")

    @sleep_and_retry
    @limits(calls=1, period=1)
    def _get_software_for_paper(self, doi: str) -> ResearchSoftware | None:
        """
        ...
        response = requests.get(f"{URL_PAPER_BASE}{doi}.json")
        ...
        if canon_project_data := canonical_project_data(orig_repo):
            ...
            if archive_url and "10.5281" in str(archive_url):
    ...
    def set_version_from_zenodo_metadata(
        ...
        version.determined_from = DeterminationSource.ARCHIVE_METADATA
        if git_platform_version_url:
            if "/tree/" in git_platform_version_url:
                version.identification_url = git_platform_version_url
                version.type = VersionType.TAG
                tag = git_platform_version_url.split("/")[-1]
                version.version = tag
            elif m := re.search(
                r"/(?:tags?|releases|commits/tag)/([^/]+)", git_platform_version_url
            ):
                version.identification_url = git_platform_version_url
                version.type = VersionType.TAG
                tag = m.group(1)
    ...
    @staticmethod
    def _get_git_platform_url(zenodo_json) -> str:
