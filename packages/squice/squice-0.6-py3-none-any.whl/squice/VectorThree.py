...
class VectorThree(object):
    def __init__(self, a=0.0, b=0.0, c=0.0, abc=[0.0, 0.0, 0.0]):
    ...
    def to_coords_str(self, rnd=4):
        return f"({round(self.A,rnd)},{round(self.B,rnd)},{round(self.C,rnd)})"
    ...
    def distance(self, ABC):
        dis = (
            (self.A - ABC.A) * (self.A - ABC.A)
            + (self.B - ABC.B) * (self.B - ABC.B)
            + (self.C - ABC.C) * (self.C - ABC.C)
        )
