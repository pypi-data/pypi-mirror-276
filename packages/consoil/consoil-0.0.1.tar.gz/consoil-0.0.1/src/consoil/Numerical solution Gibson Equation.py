# -*- coding: utf-8 -*-
"""
Created on Mon Aug 21 16:55:07 2023
...
t2c = float(input('The settling and consolidation time in hours : '))
t2c = 2*t2c*3600
...
def diffusion(phi0k, mode):
    ...
    if mode == "Zn" :
        ...
        return v0/(D0*g*(ps-pw))*((Ap)**(1/Bp) * (phi0k/(1-phi0k))**(1/Bp)+KDELsig)/max(eps,phi0k)
...
def Ddiffusion(phi0k, mode):
    ...
    if mode == "Zn" :
        KDELsig= (Ap*phi0k/(1-phi0k))**(1/Bp) - KDELsig0
        dg = v0/(D0*g*(ps-pw))*((Ap)**(1/Bp)*((1/Bp)+phi0k-1)*(phi0k)**(1/Bp - 2) / (1-phi0k)**(1/Bp + 1) + KDELsig/(max(eps,phi0k))**2)
        return (dg * phi0k) + diffusion(phi0k, mode) 
...
for k in range(0,nt) :
    ...
    divb = 0
    divt = D0*advection(Y[0],mode) * Ddiffusion(Y[1], mode) / deltaZ
    advb = 0
    advt = v0*advection(Y[0], mode)
    
    a1 = 0
    b1 = (deltaZ/deltaT) + divt
    c1 = (-advt - divt)
    dd1 = (deltaZ/deltaT)*Y[0]
    ...
    for i in range(1,kmax-1) :
        divb = D0*advection(Y[i-1],mode) * Ddiffusion(Y[i], mode) / deltaZ
        divt = D0*advection(Y[i],mode) * Ddiffusion(Y[i+1], mode) / deltaZ
        advb = v0*advection(Y[i-1], mode)
        advt = v0*advection(Y[i], mode)
        
        ai = -divb
        bi = (deltaZ/deltaT) + divt + advb + divb
        ci = (-advt - divt)
        ddi = (deltaZ/deltaT)*Y[i]
    ...
    ifinal = kmax - 1 
    divb = D0*advection(Y[ifinal-1],mode) * Ddiffusion(Y[ifinal], mode) / deltaZ
    divt = 0
    advb = v0*advection(Y[ifinal-1], mode)
    advt = 0

    af = -divb
    bf = (deltaZ/deltaT) + divt + advb +divb
    cf = (-advt - divt)
    ddf = (deltaZ/deltaT)*Y[ifinal]
    a.append(af)
    b.append(bf)
    c.append(cf)
    dd.append(ddf)
    ...
    #finding bed/suspension and water/suspension interfaces  
    for i in range(nh-2, -1, -1):
        if abs(Y[i]) > 1e-04 :
            hws.append(zbin[i])
            break

    for i in range(0,nh-1,1) : 
        epsilon = abs((Y[i] - volinit)/ volinit)
        if epsilon < 10e-01 :
            hsb.append(zbin[i])
            break
...
Settling_column(zbin, Y)
#plotting the profile phi_s (volume fraction) as a function of z for the time tn
fig_fis_z , ax_fis_z = plt.subplots()
ax_fis_z.plot(Y, zbin[:len(zbin)-1], label = 'Theoritical solution after t = '+ str(round(time[-1]/(3600),2)) + ' hours')

fig_interface , ax_interface = plt.subplots()
#plotting the profiles of the water/suspension and  suspension/bed interfaces 
ax_interface.plot(time, hws)
ax_interface.plot(time, hsb)
ax_interface.set_xlim(100,)
ax_interface.legend(['Water/Suspension hws', 'Suspension/bed hsb'])
ax_interface.set_xscale('log')
ax_interface.set_ylabel("Height in m")
ax_interface.set_xlabel("Time in sec")
ax_interface.set_title("Evolution of water/suspension and suspension/bed interfaces in time with Kk = " + str(Kk))
...
ax_fis_z.legend(['at t = '+str(t2c/(2*3600))+' h', 'at t = 0'],loc = 'center right')
