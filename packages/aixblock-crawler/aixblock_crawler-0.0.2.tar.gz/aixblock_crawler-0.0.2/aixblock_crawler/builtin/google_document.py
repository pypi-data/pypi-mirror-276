...
class GoogleDocumentFeeder(Feeder):
    ...
    def feed(self, keyword, offset, max_num, language=None, filters=None):
        ...
        # XML 	.xml
        # Image formats	BMP, GIF, JPEG, PNG, WebP, SVG
        # Video formats	3GP, 3G2, ASF, AVI, DivX, M2V, M3U, M3U8, M4V, MKV, MOV, MP4, MPEG, OGV, QVT, RAM, RM, VOB, WebM, WMV, XAP
        filetype='pdf'
...
class GoogleDocumentParser(Parser):
    def parse(self, response):
        ...
        for div in image_divs:
            ...
            uris = re.findall(r"http[^\[]*?.(?:pdf)", txt)
...
class GoogleDocumentCrawler(Crawler):

    def __init__(self,
                 feeder_cls=GoogleDocumentFeeder,
                 parser_cls=GoogleDocumentParser,
                 downloader_cls=ImageDownloader,
                 *args,
                 **kwargs):
        super(GoogleDocumentCrawler, self).__init__(
    ...
    def crawl(self,
        ...
        downloader_kwargs = dict(
            ...
            overwrite=overwrite)
        super(GoogleDocumentCrawler, self).crawl(
