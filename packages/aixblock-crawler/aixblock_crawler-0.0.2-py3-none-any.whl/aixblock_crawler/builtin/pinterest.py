...
class PinterestFeeder(Feeder):
    ...
    def feed(self, keyword, offset, max_num, filters=None):
        ...
        base_url = ('http://image.baidu.com/search/flip?tn=baiduimage&ie=utf-8&word=%s&pn={}&gsm=3c&ct=&ic=0&lm=-1&width=0&height=0')
...
class PinterestParser(Parser):
    ...
    def parse(self, response):
        try:
            import re
            img_list = re.findall('"objURL":"(.*?)",',response)
        except:
            self.logger.error('Fail to parse the response in json format')
            return
        for item in img_list:
           
            end = re.search('(.jpg|.jpeg|.png|bmp)$',pic_url)
            if end == None:
                pic_url = pic_url + '.jpg'
            try:
              yield dict(file_url=pic_url)
...
class PinterestImageCrawler(Crawler):

    def __init__(self,
                 feeder_cls=PinterestFeeder,
                 parser_cls=PinterestParser,
                 downloader_cls=ImageDownloader,
                 *args,
                 **kwargs):
        super(PinterestImageCrawler,
    ...
    def crawl(self,
        ...
        downloader_kwargs = dict(
            ...
            overwrite=overwrite)
        super(PinterestImageCrawler, self).crawl(
