...
from ..i18n import country_name, translatable_string as _, gettext
...
class BadgesPrintout:
    ...
    def getArgumentsFromRequest(cls, session, request):
        ...
        try:
            ...
            try:
                ...
                raise OperationAborted(_('No tourney with guid $id',
                                         mapping=dict(id=id)))
        else:
            entity = session.get(Tourney, idtourney)
            if entity is None:
                raise OperationAborted(_('No tourney with id $id',
    ...
    def drawLeftSide(self, competitor, player, rank):
        ...
        if self.tourney.championship.club.emblem:
            ...
            if exists(image):
                ...
                canvas.drawImage(image, 0, 0, image_width, self.height,
        ...
        canvas.drawCentredString(center, self.height-0.8*cm, self.tourney.description)

        style = cardinfo_style
        canvas.setFont(style.fontName, style.fontSize, style.leading)
        canvas.drawCentredString(center, self.height-1.4*cm,
        ...
        canvas.setFont(style.fontName, style.fontSize, style.leading)
        canvas.drawCentredString(center, self.height-2*cm,
        ...
        if self.tourney.prized:
            ...
            if self.tourney.championship.prizes == 'asis':
                ...
                canvas.drawRightString(rx+0.8*cm, self.height-4.1*cm, str(competitor.points))
                canvas.setFont(style.fontName, style.fontSize-2, style.leading)
                rx = center + 1.7*cm
                canvas.drawRightString(rx,        self.height-3.9*cm, gettext('Bucholz:'))
                canvas.drawRightString(rx+0.6*cm, self.height-3.9*cm, str(competitor.bucholz))
                canvas.drawRightString(rx,        self.height-4.2*cm, gettext('Net score:'))
                canvas.drawRightString(rx+0.6*cm, self.height-4.2*cm, str(competitor.netscore))
            else:
                rx = image_width + 1.6*cm
                canvas.drawRightString(rx,        self.height-4.2*cm, gettext('Bounty:'))
                canvas.drawRightString(rx+1*cm, self.height-4.2*cm, str(competitor.prize))
                canvas.setFont(style.fontName, style.fontSize-2, style.leading)
                rx = center + 1.7*cm
                canvas.drawRightString(rx,        self.height-3.9*cm, gettext('Points:'))
                canvas.drawRightString(rx+0.6*cm, self.height-3.9*cm, str(competitor.points))
                canvas.drawRightString(rx,        self.height-4.2*cm, gettext('Bucholz:'))
                canvas.drawRightString(rx+0.6*cm, self.height-4.2*cm, str(competitor.bucholz))
                canvas.drawRightString(rx,        self.height-4.5*cm, gettext('Net score:'))
                canvas.drawRightString(rx+0.6*cm, self.height-4.5*cm, str(competitor.netscore))
        ...
        if competitor.player1Nationality:
            ...
            if exists(flag):
                canvas.drawRightString(center-0.1*cm, self.height-5.2*cm, country)
    ...
    def drawRightSide(self, competitor, player, rank):
        ...
        canvas.setFont(cardsmall_style.fontName, cardsmall_style.fontSize,
        ...
        for i, m in enumerate(pmatches):
            ...
            canvas.drawString(0.2*cm, h,
                              e(other.caption(html=False), 2.3*cm)
                              if other else gettext('Phantom'))
            canvas.drawRightString(2.9*cm, h, str(myscore))
            canvas.drawRightString(3.3*cm, h, str(otherscore))

        canvas.drawCentredString(6.0*cm, self.height-1*cm,
                                 gettext('Final ranking'))
        canvas.drawString(3.7*cm, self.height-1.25*cm,
                          gettext('Competitor'))
        if self.tourney.championship.prizes == 'asis':
