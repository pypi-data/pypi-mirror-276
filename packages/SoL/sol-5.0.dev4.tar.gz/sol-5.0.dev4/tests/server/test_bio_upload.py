...
def test_upload_sol(lele_user, session, user_lele):
    ...
    cship = session.query(Championship).filter_by(description="Campionato CCM 2014-2015 Test").one()
    assert cship.idowner == user_lele.iduser

    tourney = session.query(Tourney).filter_by(description="Open Milano 2014-2015 Test").one()
