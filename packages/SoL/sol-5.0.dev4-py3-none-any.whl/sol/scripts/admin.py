...
def create_config(args):
    ...
    print(f'The password for the "{admin}" super user is "{password}"',
...
def initialize_db(args):
    ...
    if inspector.has_table('tourneys'):  # pragma: no cover
        print(f'Database "{engine.url}" already exist.'
...
def restore_all(args):
    ...
    print(f'Done, {len(tourneys)} new/updated tourneys, {skipped} skipped/already present.')
...
def load_historical_rating(args):
    ...
    try:
        ...
        if deviation is None:
            print(f'The formula "{args.deviation}" to compute the deviation is invalid!')
            return 128
    else:
    ...
    try:
        ...
        if volatility is None:
            print(f'The formula "{args.volatility}" to compute the volatility is invalid!')
            return 128
    else:
    ...
    if not args.dry_run:
        if (sasess.query(Rating)
                .filter_by(description=args.description)
                .one_or_none()):  # pragma: nocover
...
def _sound(which, ogg):
    ...
    target = join(dirname(dirname(__file__)), 'static', 'sounds',
                  which+'.ogg')
...
def main(args=None):
    ...
    subp.add_argument('--secret-key', default=None, help="Specify the secret key"
                      " needed to decrypt the backup")
    subp.add_argument('config', help="Name of the configuration file")
