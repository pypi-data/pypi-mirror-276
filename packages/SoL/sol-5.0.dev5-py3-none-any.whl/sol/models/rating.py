...
class Rating(GloballyUnique, Base):
    ...
    level: Mapped[str] = mapped_column(
        ...
        info=dict(label=_('Level'),
                  hint=_('Rating level.'),
                  dictionary={str(level): desc for level, desc in enumerate((
    ...
    def check_insert(
        ...
        if user_id != 'admin':
            ...
            if session.scalar(select(users.c.maxratinglevel)
                              .where(users.c.iduser == user_id)) > fields['level']:
                raise OperationAborted(_('You are not allowed to create a rating at that level'))
    ...
    def check_update(
        ...
        if 'level' in fields:
            ...
            maxratinglevel = session.scalar(select(users.c.maxratinglevel)
    ...
    def ranking(self):
        ...
        q = (select(rt.c.idplayer, rt.c.rate, rt.c.deviation, rt.c.volatility,
        ...
        return [(sasess.get_one(Player, idplayer), r, rd, rv, rc)
    ...
    def _computeExpdsOutcomes(self, score1: int, score2: int) -> tuple[float, float]:
        ...
        *before* that date.
        ...
        for tourney in self.tourneys:
            ...
            for idplayer in outcomes:
                ...
                if logger.isEnabledFor(logging.DEBUG):  # pragma: nocover
                    logger.debug('Computing new rate for %s', sasess.get(Player, idplayer))
