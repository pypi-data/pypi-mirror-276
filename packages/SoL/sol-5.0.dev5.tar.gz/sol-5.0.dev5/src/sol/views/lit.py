        ...
        def wrapper(request):
            ...
            for pname, iclass in zip(ipairs, ipairs):
                ...
                try:
                    ...
                    if iclass is Player:
                        try:
                            merged = session.query(MergedPlayer).filter_by(guid=guid).one()
                        except NoResultFound:
...
def _split_two_ints(arg: str) -> tuple[int, int]:
    try:
        ...
        raise ValueError(f'Invalid argument, expected two integers separated by a dash,'
                         f' got {arg!r}')
    return a, b
...
def country(request):
    ...
    query = (select(clubs_t.c.description,
                    ...
                    .where(or_(players_t.c.idclub == clubs_t.c.idclub,
...
def players(request):
    session = request.dbsession
    pt = Player.__table__
    query = session.execute(select(func.substr(pt.c.lastname, 1, 1),
                                pt.c.nationality,
...
def rating(request, session, entities):
    ...
    ntourneys = session.scalar(select(func.count(tt.c.idtourney))
                               .where(tt.c.idrating == rating.idrating))
    ...
    if turn is not None:
        ...
        if board is not None:
            try:
                ...
                session = request.dbsession
                match = session.query(Match).filter_by(idtourney=tourney.idtourney, turn=turn,
    ...
    if tourney.system == 'knockout':
        ...
        while ncomp >= 2:
            if ncomp == 2:
                ...
                rounds.append({
                    "name": t(_('Round of $ncompetitors',
        ...
        for p, c in enumerate(sorted(tourney.competitors,
                                     key=lambda c: c.position or -c.rate), 1):
        ...
        for match in tourney.matches:
            ...
            side2: dict[str, bool | int | str | list[str | dict]] = {
                'contestantId': ('phantom' if match.idcompetitor2 is None
...
def latest(request):
    ...
    session = request.dbsession
    tourneys = session.query(Tourney).filter_by(prized=True).order_by(Tourney.date.desc())[:n]
...
def get_countdown_elapsed(request):
    ...
    if tid is None:  # pragma: no cover
        raise HTTPNotFound()
    q = select(Tourney.countdownstarted, Tourney.duration).where(Tourney.idtourney == tid)
