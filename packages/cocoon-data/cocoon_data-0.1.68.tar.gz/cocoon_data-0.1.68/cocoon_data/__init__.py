...
class QueryWidget:
    def __init__(self, con):
        ...
        if self.tables:
            table_label = widgets.HTML(value=f"There are {len(self.tables)} tables in your database:")
            table_dropdown = widgets.Dropdown(options=self.tables, value=self.tables[0])
        ...
        self.vbox = widgets.VBox([self.dropdown_label, self.label, self.sql_input, self.submit_button, self.error_msg, self.result_display])
