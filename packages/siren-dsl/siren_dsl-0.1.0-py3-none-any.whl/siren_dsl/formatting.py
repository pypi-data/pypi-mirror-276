...
def Sankey_createLinks(source: str, targets: FSharpList[tuple[str, float]]) -> SankeyElement:
    def _arrow130(__unit: None=None, source: Any=source, targets: Any=targets) -> IEnumerable_1[SankeyElement]:
        def _arrow129(match_value: tuple[str, float]) -> IEnumerable_1[SankeyElement]:
            return singleton(SankeyElement(0, Sankey_formatLink(source, match_value[0], match_value[1])))

        return collect(_arrow129, targets)

    return SankeyElement(1, to_list(delay(_arrow130)))
...
def Block_formatEmptyBlockArrow(id: str, width: int | None, direction: BlockArrowDirection) -> str:
    def _arrow133(_arg: int, id: Any=id, width: Any=width, direction: Any=direction) -> str:
        return "&nbsp;"

    label_str: str = initialize(default_arg(width, 3), _arrow133)
    direction_str: str = Siren_BlockArrowDirection__BlockArrowDirection_ToFormatString(direction)
