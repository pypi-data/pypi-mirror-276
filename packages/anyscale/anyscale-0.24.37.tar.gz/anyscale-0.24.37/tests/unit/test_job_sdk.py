...
class TestLogs:
    def test_not_found(
        ...
        with pytest.raises(
            ...
            sdk.get_logs(job_id="does-not-exist")
    ...
    def test_basic(
        ...
        logs = (
            sdk.get_logs(name=name) if get_logs_by_name else sdk.get_logs(job_id=job_id)
        )
        ...
        logs = (
            ...
            if get_logs_by_name
            else sdk.get_logs(job_id=job_id, run=failed_run.name)
        )
        assert logs == failed_run_logs
        ...
        logs = (
            ...
            if get_logs_by_name
            else sdk.get_logs(job_id=job_id, run=succeeded_run.name)
        )
        assert logs == succeeded_run_logs
