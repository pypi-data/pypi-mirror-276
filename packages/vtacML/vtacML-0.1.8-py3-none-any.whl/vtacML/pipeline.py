...
def predict_from_best_pipeline(X, prob_flag=False, model_path='output/models/rfc_best_model.pkl', config_file='/config/config.yaml'):
    """
...
def _get_data(path=None):
    if path is None:
        with pkg_resources.path('vtacML.data', 'combined_qpo_vt_all_cases_with_GRB_with_flags.parquet') as data_path:
...
class VTACMLPipe:
    ...
    def train(self, model_path=None, resample=False, scoring='f1'):
        ...
        for name, model in models.items():
            ...
            joblib.dump(grid_search,
                        f'/output/models/{name}_grid_search.pkl')
    ...
    def load_best_model(self, model_name='rfc', model_path=None):
        if model_path is None:
            with pkg_resources.open_binary('vtacML.output.models', f'{model_name}_best_model.pkl') as file:
                self.best_model = joblib.load(file)
                logging.info(f'Loaded {model_name}_best_model.pkl')
    ...
    def _load_config(self, config_path):
        ...
        data = _get_data(path=data_path).dropna()
