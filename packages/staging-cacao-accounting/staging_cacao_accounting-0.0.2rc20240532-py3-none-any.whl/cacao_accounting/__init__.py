...
from cacao_accounting.config import DEVELOPMENT, DIRECTORIO_ARCHIVOS, DIRECTORIO_PLANTILLAS, MODO_ESCRITORIO, TESTING_MODE
...
def actualiza_variables_globales_jinja(app: Union[Flask, None] = None) -> None:
    ...
    if app and isinstance(app, Flask):
        with app.app_context():
            ...
            app.jinja_env.globals.update(TESTING_MODE=TESTING_MODE)
...
def create_app(ajustes: Union[dict, None] = None) -> Flask:
    ...
    if ajustes:
        ...
        log.debug("No se cargo configuration.")
