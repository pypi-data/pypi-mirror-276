...
def main() -> None:
    ...
    try:
        torchl_module = import_module(model_path)  # note works for pytorch lightning as well as keras
    ...
    # Setup logging file
    create_file_handler(ofname_root + '-torchl_onnx.log')
    update_console_handler(verbose)
    initial_log_messages('torchl_onnx', sonusai_torchl.__version__)
    ...
    if 'hyper_parameters' in checkpoint:
        ...
        try:
            model = torchl_module.MyHyperModel(**hparams)  # use hparams
            # torchl_module.MyHyperModel.load_from_checkpoint(ckpt_name, **hparams)
        except Exception as e:
        ...
        try:
            tmp = torchl_module.MyHyperModel()  # use default hparams
        except Exception as e:
            logger.exception(f'Error: model build (MyHyperModel) in {model_base} failed: {e}')
            raise SystemExit(1)
        ...
        model = torchl_module.MyHyperModel(timesteps=timesteps, batch_size=batch_size)
