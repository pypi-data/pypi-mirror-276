...
from djangoldp_energiepartagee.models import energy_production
...
class ProductionSite(Model):
    ...
    def technologies_used(self):
        energy_productions = self.energy_productions.filter(technology_used__isnull=False)
        if energy_productions:
            return (
                set(
                    energy_productions
                    .values_list("technology_used", flat=True)
                    .distinct()
                )
                if self.visible
                else None
    ...
    def energies_produced(self):
        energy_productions = self.energy_productions.filter(energy_type__name__isnull=False)
        if energy_productions:
            return (
                set(
                    energy_productions
                    .values_list("energy_type__name", flat=True)
                    .distinct()
                )
                if self.visible
                else None
