...
#-------Helper Functions---------------------------------------------------
    ...
    def _format_Response(self, response_json):
        ...
        if (len(dates_converted) == len(dataframe.index)):
            ...
            dataframe['Dates'] = dates_converted[0] # This populates a column named 'Dates' with single Date given 
            dataframe.index = dataframe['Dates'] # copy the column to the index column
            dataframe.index.name = 'Dates' # This will set the index column name to 'Dates' when df is formed
            dataframe = dataframe.drop(columns='Dates', axis = 1) # drop the old column 
