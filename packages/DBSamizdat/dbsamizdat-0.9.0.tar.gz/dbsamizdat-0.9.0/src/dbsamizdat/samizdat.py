...
from .util import db_object_identity, fqify_node, nodenamefmt, decode
...
class Samizdat(ABC):
    ...
    def sign(cls, cursor):
        ...
        We need the cursor to let psycopg properly escape our json-as-text-string.
        '''
        return decode(cursor.mogrify("""COMMENT ON {cls.entity_type.value} {cls.db_object_identity} IS %s;""".format(cls=cls), (cls.dbinfo(),)))
...
class SamizdatTrigger(Samizdat, ABC, metaclass=SamizdatTriggerMeta):
    ...
    @classmethod
    def sign(cls, cursor):
        return decode(cursor.mogrify("""COMMENT ON {cls.entity_type.value} "{cls.name}" ON {on_table} IS %s;""".format(cls=cls, on_table=db_object_identity(cls.on_table)), (cls.dbinfo(),)))
