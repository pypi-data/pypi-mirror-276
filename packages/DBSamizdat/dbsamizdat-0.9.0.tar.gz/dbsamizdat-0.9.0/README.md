...
It requires Python 3.8 or later, and is tested for compatibility with PostgreSQL 12 and up.
...
Simply run `pipx install --system-site-packages dbsamizdat` .
You will also need either `psycopg` or the older `psycopg2` python package; preferably one of these should be installed into your system site packages.
DBSamizdat will let you know if they are missing. If they are missing, in a pinch you could run `pipx inject dbsamizdat 'psycopg[binary]'`.
