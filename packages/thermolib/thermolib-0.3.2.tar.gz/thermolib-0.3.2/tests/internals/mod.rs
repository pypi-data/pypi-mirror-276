...
fn compare_eq(f64_short: f64, f64_long: f64) {
    if (f64_short / f64_long - f64_long / f64_short).abs() * 1E10 < 1.0 {
        return;
...
#[allow(non_snake_case)]
impl Value {
    pub fn new_short(T: f64, D: f64, p: f64, cv: f64, cp: f64, w: f64) -> Value {
        Value {
            T,
            D,
            p,
    ...
    pub fn new_long(T: f64, D: f64, p: f64, cv: f64, cp: f64, w: f64, h: f64, s: f64) -> Value {
        Value {
            T,
            D,
            p,
    ...
    pub fn test(&self, fluid: &mut HelmholtzPure) {
        // test td_flash
        fluid.td_flash(self.T, self.D).unwrap();
        ...
        if self.p != 0.0 {
            ...
            compare_eq(self.D, fluid.D().unwrap());
        }
    }
}
