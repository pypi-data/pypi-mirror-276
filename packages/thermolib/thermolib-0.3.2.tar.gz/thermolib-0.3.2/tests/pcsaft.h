...
const static double kb =
    1.380648465952442093e-23;  // Boltzmann constant, J K^-1
const static double PI = 3.141592653589793;
const static double N_AV = 6.022140857e23;  // Avagadro's number
const static double E_CHRG =
    1.6021766208e-19;  // elementary charge, units of coulomb
const static double perm_vac =
    8.854187817e-22;  // permittivity in vacuum, C V^-1 Angstrom^-1
#ifndef DBL_EPSILON
#include <limits>
...
vector<double> flashTQ_cpp(double t, double Q, vector<double> x,
                           add_args &cppargs,
                           double p_guess);  // used if a guess value is given
vector<double> flashPQ_cpp(double p, double Q, vector<double> x,
                           add_args &cppargs);
vector<double> flashPQ_cpp(double p, double Q, vector<double> x,
                           add_args &cppargs,
                           double t_guess);  // used if a guess value is given
...
inline double calc_sigma(double t, double (*function)(double)) {
  return function(t);
}  // this can allow us to accept a custom function for a temperature dependent
   // sigma
