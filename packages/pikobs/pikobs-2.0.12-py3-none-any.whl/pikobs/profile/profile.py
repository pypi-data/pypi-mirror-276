...
def create_timeserie_table(family, 
  ...
  #  print (existing_db_filename)
    new_db_conn.execute("SELECT load_extension('/home/dlo001/Plotout/libudfsqlite/src/libudfsqlite-shared.so')")
 ...
 --       SUM(BIAS_CORR) AS BiasC,
    ...
    WHERE
        ...
        {LATLONCRIT}
        {VCOCRIT}
        and  round(nominal_ppi_elevation,1) = 1.8
    GROUP BY
        range, ID_STN
  --  HAVING 
...
def create_data_list_plot(datestart1,
    ...
    for idstn in id_stns:
       print (idstn)
       if id_stn=='alone':
          criter =f'where id_stn is like  "{idstn[0]}"'
