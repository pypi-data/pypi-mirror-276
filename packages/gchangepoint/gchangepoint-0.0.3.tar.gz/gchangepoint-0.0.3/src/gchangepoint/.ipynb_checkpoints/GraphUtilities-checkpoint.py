...
def HotellingTest(data, nID, mID):
    d = data.shape[1] # dimension of the observations
    X = data[nID, :]
    Y = data[mID, :]
    
    nX = X.shape[0] # number of observations in sample 1
    nY = Y.shape[0] # number of observations in sample 2
    
    uX = np.mean(X, 0) # means of sample 1 across rows (mean of each column)
    uY = np.mean(Y, 0) # means of sample 1 across rows (mean of each column)
    uDIFF = uX-uY
    
    sX = np.cov(X.T)
    sY = np.cov(Y.T)
    s = (sX/nX)+(sY/nY)
    chi2p = np.linalg.solve(s, uDIFF)@uDIFF
    pval = chi2.sf(chi2p, df=d)
    results = {"hotelling.stat" : chi2p, "hotelling.pval" : pval}
    return(results)
