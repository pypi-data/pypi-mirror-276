...
import numpy as np
from pydantic import StrictInt, model_validator, field_validator, ValidationInfo
...
class EventComposite(BaseComposite):
    ...
    geos: Union[
        GeoComposite, xr.DataArray
    ]  # TODO xr.DataArray serialization is really needed ?
    ...
    @property
    def compute_values_to_aggregate(self):
        return (
    ...
    def _compute(self, **_kwargs) -> Optional[Union[xr.DataArray, xr.Dataset]]:
        ...
        return self.compute_downstream_aggregation(plain_da, mountain_da)
    ...
    def cover_period(self) -> List[np.datetime64]:
        ...
        """
        return list(self.field.coord(self.time_dimension).astype("datetime64[s]"))
    ...
    def get_representative_values(
        ...
        if not aggregation.method.is_after_threshold:
            ...
            prefix, _, _ = split_var_name(str(field_da.name))
    ...
    def compute_plain_and_mountain(self) -> Tuple[xr.DataArray, xr.DataArray]:
        ...
        if self.compute_values_to_aggregate:
            if self.plain is not None:
...
class EventAccumulationComposite(EventComposite):
    ...
    process: Literal["Accumulation"] = "Accumulation"
