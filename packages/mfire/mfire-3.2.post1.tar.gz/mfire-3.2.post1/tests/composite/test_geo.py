...
class TestGeoComposite:
    ...
    def test_compute(self, mock_func):
        ...
        assert geo.compute(mask_id="test") == ((), {"ids": "test"})
        assert geo.compute(mask_id=["test"]) == ((), {"ids": ["test"]})
    ...
    def test_mask_da(self, test_file_path):
        ...
        geo = GeoCompositeFactory(file=test_file_path, grid_name="A")
    ...
    @pytest.mark.parametrize("test_file_path", [{"extension": "nc"}], indirect=True)
    def test_all_axis(self, test_file_path):
        ds = xr.Dataset(
            {
                "A": (
    ...
    @pytest.mark.parametrize("test_file_path", [{"extension": "nc"}], indirect=True)
    def test_all_sub_areas(self, test_file_path):
        ds = xr.Dataset(
            {
                "A": (
