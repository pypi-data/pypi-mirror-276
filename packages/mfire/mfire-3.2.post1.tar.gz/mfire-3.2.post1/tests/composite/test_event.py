...
from mfire.composite.event import Category, EventComposite, Threshold
...
class TestEventComposite:
    ...
    def test_compute_values_to_aggregate(self, event, expected):
        evt = EventCompositeFactory(**event)
        assert evt.compute_values_to_aggregate == expected
    ...
    def test_field_da(self):
        ...
        event = EventCompositeFactory(
            field=FieldCompositeFactory(data_factory=field_da)
        )
        assert_identically_close(
            event.field_da,
            xr.DataArray([[[0.3, 1.5, 2.5, 2.4]]], coords=coords, name="field_name"),
        )
    ...
    def test_compute_plain_and_mountain(self, event, assert_equals_result):
        ...
        evt = EventCompositeFactory(
            **event,
            field=FieldCompositeFactory(data_factory=field_da),
            altitude=AltitudeCompositeFactory(data_factory=altitude_da),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"aggregation": None, "compute_list": ["representative"]},
            {
                "aggregation": None,
                "plain": None,
                "mountain": Threshold(units="mm", threshold=0, comparison_op="sup"),
    ...
    def test_compute_downstream_aggregation(self, event, assert_equals_result):
        lon, lat = [15], [30, 31, 32]
        valid_time = [Datetime(2023, 3, i).as_np_dt64 for i in range(1, 4)]
        coords = {"longitude": lon, "latitude": lat, "valid_time": valid_time}

        field = FieldCompositeFactory(
            data_factory=xr.DataArray(None, name="NEIPOT24__SOL")
        )
        ...
        result = evt.compute_downstream_aggregation(plain_da, mountain_da)
        assert_equals_result(
...
class TestEventAccumulationComposite:
    ...
    def test_field_da(self, cum_period, expected_rolling_values):
        ...
        field = FieldCompositeFactory(data_factory=field_da)
        field_1 = FieldCompositeFactory(data_factory=field_1_da)
