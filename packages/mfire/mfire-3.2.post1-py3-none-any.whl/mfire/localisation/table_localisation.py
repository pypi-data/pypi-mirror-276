...
class TableLocalisation(BaseModel):
    ...
    data: xr.DataArray  # TODO serialization ?
    domain: xr.DataArray
    areas: xr.DataArray

    request_time: s_datetime
    ...
    def _merge_similar_areas(self, da: xr.DataArray) -> xr.DataArray:
        ...
        areas = self.areas.sel(id=areas_id)
        ...
        if compute_IoU(areas.max("id"), self.domain) > 0.98:
            # If the IoU is greater than 0.98, then use the name and type of the
            # domain.
            area_name = self.domain.areaName.item()
            area_type = "domain"
