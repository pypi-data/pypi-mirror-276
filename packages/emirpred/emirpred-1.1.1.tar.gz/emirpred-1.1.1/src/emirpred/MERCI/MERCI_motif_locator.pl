...
my $line = '';  # Initialize $line to avoid uninitialized value errors
while (<IN>) {
    $line = $_;  # Read a line from the file
    last if $line =~ /Motifs:/;  # Exit the loop if the line contains "Motifs:"
}

# Check if the string "Motifs:" was not found
if ($line !~ /Motifs:/) {
    die "The string 'Motifs:' was not found in the input file.";
}

# Process the remaining lines in the file
while (<IN>) {
    chomp;
    my $pattern = &pattern_to_string($_);
    &check_locations($_, $pattern);
}
