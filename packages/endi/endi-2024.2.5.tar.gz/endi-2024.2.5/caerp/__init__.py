...
def config_services(config):
    """
    Setup the services (pyramid_services) used in de enDI
    """
...
def add_static_views(config, settings):
    """
    Add the static views used in de enDI
    """
...
def add_base_directives_and_predicates(config):
    """
    Add custom predicates and directives used in de enDI's codebase
    """
...
def version(strip_suffix=False) -> str:
    """
    Return de enDI's version number (as defined in setup.py)
