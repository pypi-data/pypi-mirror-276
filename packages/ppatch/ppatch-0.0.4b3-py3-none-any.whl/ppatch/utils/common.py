...
def clean_repo():
    output = subprocess.run(["git", "clean", "-df"], capture_output=True).stdout.decode("utf-8")
    logger.debug(output)

    output = subprocess.run(["git", "reset", "--hard"], capture_output=True).stdout.decode("utf-8")
    logger.debug(output)
